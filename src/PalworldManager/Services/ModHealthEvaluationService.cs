...
public sealed class ModHealthEvaluationService
{
        ...
        bool duplicateDetected = false)
    {
        ...
        // UE4SS mods require positive runtime evidence for a Confirmed Loaded/Running claim.
        if (isUe4ss)
        {
            ...
            return Result(ModHealthStatus.RuntimeUnverified, 70, "Active / Unverified",
                serverRunning
                    ? "Installed, enabled, and present in the Active Mods Root. No positive mod-specific runtime signature has been observed; this is not proof of a load failure."
                    : "Installed and enabled, but runtime execution cannot be confirmed while the server is offline.");
        }

        // PAK/Workshop packages do not require a UE4SS 'Starting Lua mod' entry.
        return Result(ModHealthStatus.Healthy, 100, "Healthy",
            "Installation and enabled-state verification passed. UE4SS runtime load evidence is not required for this MOD type.");
    }
    ...
    public static string ToDisplayText(ModHealthStatus status) => status switch
    {
        ...
        ModHealthStatus.RuntimeUnverified => "Active / Unverified",
