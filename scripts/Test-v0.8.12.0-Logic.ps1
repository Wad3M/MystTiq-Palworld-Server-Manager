...
$ctx = New-MystTiqTestContext -ProjectRoot $root -Version '0.8.12.0' -Suite 'Logic'

# ---------------------------------------------------------------------------
# 1. Version / build identity
# ---------------------------------------------------------------------------
Test-MystTiqTextMatch $ctx 'Directory.Build.props' `
    '<VersionPrefix>0\.8\.12\.0</VersionPrefix>' `
    'Versioning' 'Directory.Build.props reports v0.8.12.0' -Severity Critical

Test-MystTiqTextMatch $ctx 'src\PalworldManager\app.manifest' `
    'assemblyIdentity version="0\.8\.12\.0"' `
    'Versioning' 'app.manifest reports v0.8.12.0' -Severity Critical
...
Test-MystTiqFile $ctx 'scripts\Test-v0.8.11.0-Logic.ps1' 'Regression' 'v0.8.11.0 logic gate remains available' -Severity High
...
$imageRows = @([regex]::Matches($imageRecord, '\| [123] \| `(\w+)\.png` \| [^|]+ \| `([0-9A-F]{64})` \| `([0-9A-F]{64})` \|') | ForEach-Object { [pscustomobject]@{ Name = $_.Groups[1].Value; Hash = $_.Groups[2].Value } })
...
Add-MystTiqCheck $ctx 'Regression' 'v0.8.10.0 Ribbon images remain: all 30 are the recorded originals (shipped hashes match, original hashes recorded)' `
    ($imageRows.Count -eq 30 -and $imageBad.Count -eq 0 -and @(Get-ChildItem $imageDir -Filter *.png).Count -eq 30) `
...
# ---------------------------------------------------------------------------
# 3. v0.8.12.0 Contracts -- a second NAT (CGNAT / double NAT) in the WAN reachability check
# ---------------------------------------------------------------------------
...
Add-MystTiqCheck $ctx 'v0.8.12.0 Contracts' 'the router WAN address decides: 100.64/10 is CGNAT (Fail), a private address is double NAT (Fail), a different public address is a Warning' `
...
Add-MystTiqCheck $ctx 'v0.8.12.0 Contracts' 'without a router WAN address the route out is weak evidence: a private hop is "cannot tell" (it was the ISP on the real network), a silent hop 2 gives no Pass, never a Fail' `
...
Add-MystTiqCheck $ctx 'v0.8.12.0 Contracts' 'UPnP works on Linux and on multi-interface Windows: http-only control URLs, a search from every LAN interface, default-payload pings, a refused WAN address asked twice' `
...
Add-MystTiqCheck $ctx 'v0.8.12.0 Contracts' 'the service asks the router for its WAN address (GetExternalIPAddress), falls back to the first 4 hops, and never claims outside reachability' `
...
Add-MystTiqCheck $ctx 'v0.8.12.0 Contracts' 'the Desktop shows the router''s internet address and explains what is and is not checked' `
...
$harness = Get-MystTiqText $ctx 'scripts\Testing\MystTiq.LogicHarness\Program.cs'
Add-MystTiqCheck $ctx 'v0.8.12.0 Contracts' 'the logic harness covers every WAN-address and route branch, including the range edges' `
...
Add-MystTiqCheck $ctx 'v0.8.12.0 Contracts' 'the second-NAT smoke uses its own FleetRoot and checks the contract, not a network-dependent verdict' `
    ($smoke -match '\$cfg\.FleetRoot = \$fleetRoot' -and $smoke -match 'checks the contract, not a verdict') -Severity Critical
Test-MystTiqFile $ctx 'docs\architecture\v0.8.12.0-second-nat.md' 'v0.8.12.0 Contracts' 'v0.8.12.0 architecture doc is present' -Severity Critical

# ---------------------------------------------------------------------------
# 4. Documentation
# ---------------------------------------------------------------------------
$docText = (Get-ChildItem (Join-Path $root 'docs') -Recurse -File -Include *.md | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n"
Add-MystTiqCheck $ctx 'Documentation' 'v0.8.12.0 is documented' ([regex]::IsMatch($docText, 'v0\.8\.12\.0')) -Severity High
Test-MystTiqFile $ctx 'release-notes\v0.8.12.0.md' 'Documentation' 'v0.8.12.0 release notes exist' -Severity High
$changelogText = Get-MystTiqText $ctx 'CHANGELOG.md'
Add-MystTiqCheck $ctx 'Documentation' 'CHANGELOG.md has a v0.8.12.0 entry' ($changelogText -match '## v0\.8\.12\.0') -Severity High
...
# ---------------------------------------------------------------------------
if ($RunBuild) {
    $frozenZip = if ($FrozenBaselineZip) { $FrozenBaselineZip } else { Join-Path $root '..\_Backups\MystTiqPalworldServer\v0.8.11.0\MystTiqPalworldServer_v0.8.11.0_FullSource.zip' }
    if (-not (Test-Path $frozenZip -PathType Leaf)) {
        # v0.8.1.0: reported, not fatal: the rest of the gate is still worth running on a machine without it.
        Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.11.0 checkpoint logic gate still passes' $false -Skipped -Severity Critical `
            -Details "Baseline archive not found at $frozenZip. Run on the release machine, or pass -FrozenBaselineZip <path>."
    }
    else {
    $frozenRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.11.0-frozen-baseline'
    ...
    $frozenGateLog = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.11.0-frozen-gate-output.txt'
    try {
        & (Join-Path $frozenProjectRoot 'scripts\Test-v0.8.11.0-Logic.ps1') -ProjectRoot $frozenProjectRoot *> $frozenGateLog
    } catch { }
    ...
    Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.11.0 checkpoint logic gate still passes' `
    ...
    Test-MystTiqCommand $ctx 'v0.8.12.0 Runtime' 'v0.8.12.0 second NAT route smoke passes' {
    ...
    Test-MystTiqCommand $ctx 'Regression Runtime' 'MystTiq.ArtworkHarness passes (offline headless rendering, incl. the original Ribbon images on 13 pages in dark and light)' {
