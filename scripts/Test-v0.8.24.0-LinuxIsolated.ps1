...
# v0.8.24.0: an isolated MystTiq on the Linux test VM for processor cores (affinity on every thread). It publishes
# the linux-x64 headless, copies it with Test-v0.8.24.0-LinuxIsolated.sh to a temp folder on the VM, runs the check
# there (own config, FleetRoot, runtime and port 18425, a stand-in server process inside a fake server folder; the
...
$work = Join-Path $root ("artifacts\linux-isolated\v0.8.24.0-" + [guid]::NewGuid().ToString('N'))
...
$sh = [IO.File]::ReadAllText((Join-Path $root 'scripts\Test-v0.8.24.0-LinuxIsolated.sh')).Replace("`r`n", "`n")
...
$dir = '/tmp/mysttiq-v0824-isolated'
...
$lines = @(ssh @sshArgs $remote "cd $dir && tar -xzf linux-isolated.tgz && bash check.sh $version; echo exit=`$?; cd /; rm -rf $dir" 2>&1)
$lines | ForEach-Object { "$_" -replace '\b(\d{1,3}\.){3}\d{1,3}\b', '<ip>' }
Remove-Item -LiteralPath $work -Recurse -Force
if (-not ($lines -match '^exit=0$')) { throw 'MystTiq v0.8.24.0 Linux isolated check failed.' }
Write-Host 'MystTiq v0.8.24.0 Linux isolated check passed.' -ForegroundColor Green
