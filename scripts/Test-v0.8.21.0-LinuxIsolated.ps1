...
# v0.8.21.0: an isolated MystTiq on the Linux test VM for the systemd unit (LimitNICE, printed by service-unit, checked by systemd-analyze). It publishes
# the linux-x64 headless, copies it with Test-v0.8.21.0-LinuxIsolated.sh to a temp folder on the VM, runs the check
# there (own config, FleetRoot, runtime and no port, nothing installed; the
...
$work = Join-Path $root ("artifacts\linux-isolated\v0.8.21.0-" + [guid]::NewGuid().ToString('N'))
...
$sh = [IO.File]::ReadAllText((Join-Path $root 'scripts\Test-v0.8.21.0-LinuxIsolated.sh')).Replace("`r`n", "`n")
...
$dir = '/tmp/mysttiq-v0821-isolated'
...
if (-not ($lines -match '^exit=0$')) { throw 'MystTiq v0.8.21.0 Linux isolated check failed.' }
Write-Host 'MystTiq v0.8.21.0 Linux isolated check passed.' -ForegroundColor Green
