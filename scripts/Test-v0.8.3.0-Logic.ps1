...
$ctx = New-MystTiqTestContext -ProjectRoot $root -Version '0.8.3.0' -Suite 'Logic'

# ---------------------------------------------------------------------------
# 1. Version / build identity
# ---------------------------------------------------------------------------
Test-MystTiqTextMatch $ctx 'Directory.Build.props' `
    '<VersionPrefix>0\.8\.3\.0</VersionPrefix>' `
    'Versioning' 'Directory.Build.props reports v0.8.3.0' -Severity Critical

Test-MystTiqTextMatch $ctx 'src\PalworldManager\app.manifest' `
    'assemblyIdentity version="0\.8\.3\.0"' `
    'Versioning' 'app.manifest reports v0.8.3.0' -Severity Critical
...
Test-MystTiqFile $ctx 'scripts\Test-v0.8.2.0-Logic.ps1' 'Regression' 'v0.8.2.0 logic gate remains available' -Severity High
...
# ---------------------------------------------------------------------------
# 3. v0.8.3.0 Contracts -- Give Item picker
# ---------------------------------------------------------------------------
$reader = Get-MystTiqText $ctx 'src\MystTiq.HeadlessHost\SaveGameIdReader.cs'
$catalog = Get-MystTiqText $ctx 'src\MystTiq.HeadlessHost\HeadlessGameIdCatalogService.cs'
$kitService = Get-MystTiqText $ctx 'src\MystTiq.HeadlessHost\HeadlessKitService.cs'
$search = Get-MystTiqText $ctx 'src\MystTiq.Core\Services\GameIdSearch.cs'
$kitText = Get-MystTiqText $ctx 'src\MystTiq.Core\Services\KitEntryText.cs'
$vm = Get-MystTiqText $ctx 'src\MystTiq.Desktop\ViewModels\MainWindowViewModel.cs'

Add-MystTiqCheck $ctx 'v0.8.3.0 Contracts' 'ids come from the world save itself (streamed static_id and CharacterID), filtered by the give validation''s id rule' `
    ($reader -match 'reader\.ValueTextEquals\("static_id"u8\)' -and $reader -match 'reader\.ValueTextEquals\("CharacterID"u8\)' -and
     $reader -match 'private const string AlphaPrefix = "BOSS_";' -and $reader -match '\[GeneratedRegex\("\^\[A-Za-z0-9_\]\{1,64\}\$"\)\]') `
    -Severity Critical

Add-MystTiqCheck $ctx 'v0.8.3.0 Contracts' 'the catalogue merges world, kit and earlier-give ids, caches until the save changes, and is a read-only Operator route' `
    ($catalog -match 'public static IReadOnlyList<GameIdCatalogEntry> Merge\(SaveGameIds\? world, IEnumerable<KitEntry> kitEntries, IEnumerable<KitEntry> given\)' -and
     $catalog -match 'if \(cachedKey == key && cachedIds is not null\) return cachedIds;' -and
     $apiHost -match 'routes\.MapGet\("/players/give/catalog", \(\) => Results\.Ok\(p\.GameIds\.GetCatalog\(\)\)\)\s*\.RequireRole\(MystTiqRole\.Operator, p\.Id\);') `
    -Severity Critical

Add-MystTiqCheck $ctx 'v0.8.3.0 Contracts' 'only a manual give that was delivered without an error reply is remembered, capped' `
    ($kitService -match 'RecordGiven\(adHoc\.Entries\);\s*var summary' -and $kitService -match 'public const int MaximumGivenHistory = 200;' -and
     $kitService -match 'givenPath = Path\.Combine\(root, "give-history\.json"\);') `
    -Severity Critical

Add-MystTiqCheck $ctx 'v0.8.3.0 Contracts' 'search, appending a line and the amount/level limits are shared Core logic matching the server''s validation' `
    ($search -match 'public static bool Matches\(string\? id, string\? query\)' -and
     $kitText -match 'public static string Append\(string\? text, KitTextEntry entry\)' -and
     $kitText -match 'var \(label, max\) = isPal \? \("Level", 100\) : \("Amount", 1_000_000\);') `
    -Severity Critical

Add-MystTiqCheck $ctx 'v0.8.3.0 Contracts' 'the Desktop picker adds to the Give box or the selected kit, and another server''s ids are cleared on a tab switch' `
    ($xaml -match 'x:Name="GameIdPicker"' -and $xaml -match 'Command="\{Binding AddGameIdToGiveCommand\}"' -and $xaml -match 'Command="\{Binding AddGameIdToKitCommand\}"' -and
     $xaml -match 'Text="\{Binding GameIdAmountLabel\}"' -and
     $vm -match 'GiveItemsText = KitEntryText\.Append\(GiveItemsText, line\);' -and $vm -match 'KitEntriesText = KitEntryText\.Append\(KitEntriesText, line\);' -and
     $vm -match '_lastExplorerPlayers = \[\];\s*//[^\n]*\n[^\n]*\n\s*ClearGameIdCatalog\(\);') `
    -Severity Critical

$harness = Get-MystTiqText $ctx 'scripts\Testing\MystTiq.LogicHarness\Program.cs'
Add-MystTiqCheck $ctx 'v0.8.3.0 Contracts' 'the logic harness covers the reader, merging, search/append/limits, give history and the catalogue service' `
    (([regex]::Matches($harness, 'RunScenario\("Give Item picker: ')).Count -eq 5) `
    -Severity Critical

$smoke = Get-MystTiqText $ctx 'scripts\Test-v0.8.3.0-RouteSmoke.ps1'
Add-MystTiqCheck $ctx 'v0.8.3.0 Contracts' 'the catalogue smoke uses its own FleetRoot and a synthetic save (no real player data)' `
    ($smoke -match '\$cfg\.FleetRoot = \$fleetRoot' -and $smoke -match 'synthetic decoded world save') `
    -Severity Critical
Test-MystTiqFile $ctx 'docs\architecture\v0.8.3.0-give-item-picker.md' 'v0.8.3.0 Contracts' 'v0.8.3.0 architecture doc is present' -Severity Critical

# ---------------------------------------------------------------------------
# 4. Documentation
# ---------------------------------------------------------------------------
$docText = (Get-ChildItem (Join-Path $root 'docs') -Recurse -File -Include *.md | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n"
Add-MystTiqCheck $ctx 'Documentation' 'v0.8.3.0 is documented' ([regex]::IsMatch($docText, 'v0\.8\.3\.0')) -Severity High
Test-MystTiqFile $ctx 'release-notes\v0.8.3.0.md' 'Documentation' 'v0.8.3.0 release notes exist' -Severity High
$changelogText = Get-MystTiqText $ctx 'CHANGELOG.md'
Add-MystTiqCheck $ctx 'Documentation' 'CHANGELOG.md has a v0.8.3.0 entry' ($changelogText -match '## v0\.8\.3\.0') -Severity High
...
# ---------------------------------------------------------------------------
if ($RunBuild) {
    $frozenZip = if ($FrozenBaselineZip) { $FrozenBaselineZip } else { Join-Path $root '..\_Backups\MystTiqPalworldServer\v0.8.2.0\MystTiqPalworldServer_v0.8.2.0_FullSource.zip' }
    if (-not (Test-Path $frozenZip -PathType Leaf)) {
        # v0.8.1.0: reported, not fatal: the rest of the gate is still worth running on a machine without it.
        Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.2.0 checkpoint logic gate still passes' $false -Skipped -Severity Critical `
            -Details "Baseline archive not found at $frozenZip. Run on the release machine, or pass -FrozenBaselineZip <path>."
    }
    else {
    $frozenRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.2.0-frozen-baseline'
    ...
    $frozenGateLog = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.2.0-frozen-gate-output.txt'
    try {
        & (Join-Path $frozenProjectRoot 'scripts\Test-v0.8.2.0-Logic.ps1') -ProjectRoot $frozenProjectRoot *> $frozenGateLog
    } catch { }
    ...
    Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.2.0 checkpoint logic gate still passes' `
    ...
    # v0.8.3.0: the Give Item picker's catalogue route on the published exe (synthetic save, large-save timing, cache).
    Test-MystTiqCommand $ctx 'v0.8.3.0 Runtime' 'v0.8.3.0 give item catalogue route smoke passes' {
    ...
    Test-MystTiqCommand $ctx 'Regression Runtime' 'MystTiq.ArtworkHarness passes (offline headless rendering, incl. the Ribbon icons)' {
