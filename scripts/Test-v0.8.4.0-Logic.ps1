...
$ctx = New-MystTiqTestContext -ProjectRoot $root -Version '0.8.4.0' -Suite 'Logic'

# ---------------------------------------------------------------------------
# 1. Version / build identity
# ---------------------------------------------------------------------------
Test-MystTiqTextMatch $ctx 'Directory.Build.props' `
    '<VersionPrefix>0\.8\.4\.0</VersionPrefix>' `
    'Versioning' 'Directory.Build.props reports v0.8.4.0' -Severity Critical

Test-MystTiqTextMatch $ctx 'src\PalworldManager\app.manifest' `
    'assemblyIdentity version="0\.8\.4\.0"' `
    'Versioning' 'app.manifest reports v0.8.4.0' -Severity Critical
...
Test-MystTiqFile $ctx 'scripts\Test-v0.8.3.0-Logic.ps1' 'Regression' 'v0.8.3.0 logic gate remains available' -Severity High
...
# ---------------------------------------------------------------------------
# 3. v0.8.4.0 Contracts -- pause outside delivery (Discord, email, webhooks)
# ---------------------------------------------------------------------------
...
Add-MystTiqCheck $ctx 'v0.8.4.0 Contracts' 'while paused, Dispatch skips every outside channel and writes the skip to the Activity log (the notification itself is already kept)' `
    ($routing -match 'if \(NotificationDeliveryPolicy\.IsPaused\(pausedUntil, DateTimeOffset\.UtcNow\)\)' -and
     $routing -match '"Not sent outside \(delivery paused\)"') `
    -Severity Critical

Add-MystTiqCheck $ctx 'v0.8.4.0 Contracts' 'the pause is stored apart from channels.json, set on the host clock and capped like the alert mute' `
    ($routing -match 'deliveryPath = Path\.Combine\(root, "delivery\.json"\);' -and
     $routing -match 'deliveryPausedUntilUtc = AlertMutePolicy\.MuteUntil\(minutes, DateTimeOffset\.UtcNow\);') `
    -Severity Critical

Add-MystTiqCheck $ctx 'v0.8.4.0 Contracts' 'the routes: reading the state, pausing/resuming and the test notification (Admin for the two that act)' `
    ($apiHost -match 'routes\.MapGet\("/notifications/delivery"' -and
     $apiHost -match 'routes\.MapPost\("/notifications/delivery/pause", \(NotificationDeliveryPauseRequest request\) =>\s*Results\.Ok\(p\.NotificationRouting\.PauseDelivery\(request\.Minutes\)\)\)\.RequireRole\(MystTiqRole\.Admin, p\.Id\);' -and
     $apiHost -match 'routes\.MapPost\("/notifications/test"' -and $apiHost -match 'p\.Notifications\.Create\("Information", "Test notification",') `
    -Severity Critical

Add-MystTiqCheck $ctx 'v0.8.4.0 Contracts' 'the Alert Center card pauses for 1 h to 7 days, resumes, and sends a test; its state loads with the Alert Center' `
    ($xaml -match 'x:Name="PauseDeliveryCard"' -and $xaml -match 'Command="\{Binding PauseDeliveryCommand\}" CommandParameter="10080"' -and
     $xaml -match 'Content="Resume" Command="\{Binding PauseDeliveryCommand\}" CommandParameter="0" IsVisible="\{Binding NotificationDelivery\.IsPaused\}"' -and
     $xaml -match 'Command="\{Binding SendTestNotificationCommand\}"' -and
     $vm -match 'NotificationDelivery = await _api\.GetNotificationDeliveryAsync\(SelectedProfile, BearerToken\);') `
    -Severity Critical

$harness = Get-MystTiqText $ctx 'scripts\Testing\MystTiq.LogicHarness\Program.cs'
Add-MystTiqCheck $ctx 'v0.8.4.0 Contracts' 'the logic harness covers the pause policy and a real webhook receiving nothing while paused' `
    (([regex]::Matches($harness, 'RunScenario\("Delivery pause: ')).Count -eq 2 -and $harness -match 'new HttpListener\(\)') `
    -Severity Critical

$smoke = Get-MystTiqText $ctx 'scripts\Test-v0.8.4.0-RouteSmoke.ps1'
Add-MystTiqCheck $ctx 'v0.8.4.0 Contracts' 'the delivery smoke uses its own FleetRoot and a localhost-only webhook receiver' `
    ($smoke -match '\$cfg\.FleetRoot = \$fleetRoot' -and $smoke -match '\$hookUrl = "http://localhost:\$hookPort/hook/"') `
    -Severity Critical
Test-MystTiqFile $ctx 'docs\architecture\v0.8.4.0-delivery-pause.md' 'v0.8.4.0 Contracts' 'v0.8.4.0 architecture doc is present' -Severity Critical

# ---------------------------------------------------------------------------
# 4. Documentation
# ---------------------------------------------------------------------------
$docText = (Get-ChildItem (Join-Path $root 'docs') -Recurse -File -Include *.md | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n"
Add-MystTiqCheck $ctx 'Documentation' 'v0.8.4.0 is documented' ([regex]::IsMatch($docText, 'v0\.8\.4\.0')) -Severity High
Test-MystTiqFile $ctx 'release-notes\v0.8.4.0.md' 'Documentation' 'v0.8.4.0 release notes exist' -Severity High
$changelogText = Get-MystTiqText $ctx 'CHANGELOG.md'
Add-MystTiqCheck $ctx 'Documentation' 'CHANGELOG.md has a v0.8.4.0 entry' ($changelogText -match '## v0\.8\.4\.0') -Severity High
...
# ---------------------------------------------------------------------------
if ($RunBuild) {
    $frozenZip = if ($FrozenBaselineZip) { $FrozenBaselineZip } else { Join-Path $root '..\_Backups\MystTiqPalworldServer\v0.8.3.0\MystTiqPalworldServer_v0.8.3.0_FullSource.zip' }
    if (-not (Test-Path $frozenZip -PathType Leaf)) {
        # v0.8.1.0: reported, not fatal: the rest of the gate is still worth running on a machine without it.
        Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.3.0 checkpoint logic gate still passes' $false -Skipped -Severity Critical `
            -Details "Baseline archive not found at $frozenZip. Run on the release machine, or pass -FrozenBaselineZip <path>."
    }
    else {
    $frozenRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.3.0-frozen-baseline'
    ...
    $frozenGateLog = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.3.0-frozen-gate-output.txt'
    try {
        & (Join-Path $frozenProjectRoot 'scripts\Test-v0.8.3.0-Logic.ps1') -ProjectRoot $frozenProjectRoot *> $frozenGateLog
    } catch { }
    ...
    Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.3.0 checkpoint logic gate still passes' `
    ...
    # v0.8.4.0: pausing outside delivery on the published exe, with a real localhost webhook receiver.
    Test-MystTiqCommand $ctx 'v0.8.4.0 Runtime' 'v0.8.4.0 delivery pause route smoke passes' {
