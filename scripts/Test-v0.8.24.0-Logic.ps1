...
$ctx = New-MystTiqTestContext -ProjectRoot $root -Version '0.8.24.0' -Suite 'Logic'

# ---------------------------------------------------------------------------
# 1. Version / build identity
# ---------------------------------------------------------------------------
Test-MystTiqTextMatch $ctx 'Directory.Build.props' `
    '<VersionPrefix>0\.8\.24\.0</VersionPrefix>' `
    'Versioning' 'Directory.Build.props reports v0.8.24.0' -Severity Critical

Test-MystTiqTextMatch $ctx 'src\PalworldManager\app.manifest' `
    'assemblyIdentity version="0\.8\.24\.0"' `
    'Versioning' 'app.manifest reports v0.8.24.0' -Severity Critical
...
Test-MystTiqFile $ctx 'scripts\Test-v0.8.23.0-Logic.ps1' 'Regression' 'v0.8.23.0 logic gate remains available' -Severity High
...
$gateSelf821 = Get-MystTiqText $ctx 'scripts\Test-v0.8.24.0-Logic.ps1'
...
# ---------------------------------------------------------------------------
# 3. v0.8.24.0 Contracts -- processor cores, and the policy applied at start
# ---------------------------------------------------------------------------
...
Add-MystTiqCheck $ctx 'v0.8.24.0 Contracts' 'the policy has cores, checked against the machine (at most 64); the service pins only what differs and gives every core back only where MystTiq pinned; Windows sets the mask, Linux every thread' `
...
Add-MystTiqCheck $ctx 'v0.8.24.0 Contracts' 'every start and restart through MystTiq applies the policy as soon as it succeeds (never failing the start); the systemd unit keeps no Nice=' `
...
Add-MystTiqCheck $ctx 'v0.8.24.0 Contracts' 'the HOST tab edits the cores (read-only below Admin, in the priority card) and shows each process''s cores' `
...
Add-MystTiqCheck $ctx 'v0.8.24.0 Contracts' 'the harness, the Windows smoke (real process, a restart) and the Linux check (every thread) cover it' `
...
Test-MystTiqFile $ctx 'docs\architecture\v0.8.24.0-processor-cores.md' 'v0.8.24.0 Contracts' 'v0.8.24.0 architecture doc is present' -Severity Critical

# ---------------------------------------------------------------------------
# 4. Documentation
# ---------------------------------------------------------------------------
$docText = (Get-ChildItem (Join-Path $root 'docs') -Recurse -File -Include *.md | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n"
Add-MystTiqCheck $ctx 'Documentation' 'v0.8.24.0 is documented' ([regex]::IsMatch($docText, 'v0\.8\.24\.0')) -Severity High
Test-MystTiqFile $ctx 'release-notes\v0.8.24.0.md' 'Documentation' 'v0.8.24.0 release notes exist' -Severity High
$changelogText = Get-MystTiqText $ctx 'CHANGELOG.md'
Add-MystTiqCheck $ctx 'Documentation' 'CHANGELOG.md has a v0.8.24.0 entry' ($changelogText -match '## v0\.8\.24\.0') -Severity High
...
# ---------------------------------------------------------------------------
if ($RunBuild) {
    $frozenZip = if ($FrozenBaselineZip) { $FrozenBaselineZip } else { Join-Path $root '..\_Backups\MystTiqPalworldServer\v0.8.23.0\MystTiqPalworldServer_v0.8.23.0_FullSource.zip' }
    if (-not (Test-Path $frozenZip -PathType Leaf)) {
        # v0.8.1.0: reported, not fatal: the rest of the gate is still worth running on a machine without it.
        Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.23.0 checkpoint logic gate still passes' $false -Skipped -Severity Critical `
            -Details "Baseline archive not found at $frozenZip. Run on the release machine, or pass -FrozenBaselineZip <path>."
    }
    else {
    $frozenRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.23.0-frozen-baseline'
    ...
    $frozenGateLog = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.23.0-frozen-gate-output.txt'
    try {
        & (Join-Path $frozenProjectRoot 'scripts\Test-v0.8.23.0-Logic.ps1') -ProjectRoot $frozenProjectRoot *> $frozenGateLog
    } catch { }
    ...
    Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.23.0 checkpoint logic gate still passes' `
    ...
    Test-MystTiqCommand $ctx 'v0.8.24.0 Runtime' 'v0.8.24.0 cores smoke passes (real process affinity, applied as soon as a restart returns)' {
    ...
    Test-MystTiqCommand $ctx 'Regression New Tests' 'MystTiq.ArtworkHarness still passes (offline headless rendering, every gated command and page control)' {
    ...
    if ($reachable) {
        ...
        Test-MystTiqCommand $ctx 'v0.8.24.0 Runtime' 'processor cores on Linux: every thread pinned and given back (VM, isolated)' {
        ...
        Add-MystTiqCheck $ctx 'v0.8.24.0 Runtime' 'processor cores on Linux: every thread pinned and given back (VM, isolated)' $false -Skipped -Severity High -Details "The Linux test VM ($remoteHost) does not answer on SSH."
