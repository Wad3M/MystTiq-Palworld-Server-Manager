#!/usr/bin/env bash
# v0.8.12.0: an isolated MystTiq instance on the Linux test VM, run by Test-v0.8.12.0-LinuxIsolated.ps1. Own config,
# FleetRoot, runtime and port (18412) over a synthetic server folder; never touches /etc/mysttiq or the installed
# mysttiq-palworld service. Checks what v0.8.9.0 (crash reports), v0.8.11.0 (Pal positions) and v0.8.12.0 (second-NAT
# check) added. The WAN report is taken twice because the real router answers its WAN address intermittently.
set -uo pipefail
...
[[ "$V" == "0.8.12.0" ]] && ok "healthz reports $V" || bad "healthz" "version '$V' $(tail -3 "$T/headless.err.log")"
...
[[ "$P" == "PinkCat/BaseWorker/Linux Guild WeaselDragon/Party/Keeper palbox=1" ]] && ok "v0.8.11.0 Pal positions ($P)" || bad "v0.8.11.0 Pal positions" "$P"
