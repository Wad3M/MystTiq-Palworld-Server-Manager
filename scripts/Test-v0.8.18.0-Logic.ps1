...
$ctx = New-MystTiqTestContext -ProjectRoot $root -Version '0.8.18.0' -Suite 'Logic'

# ---------------------------------------------------------------------------
# 1. Version / build identity
# ---------------------------------------------------------------------------
Test-MystTiqTextMatch $ctx 'Directory.Build.props' `
    '<VersionPrefix>0\.8\.18\.0</VersionPrefix>' `
    'Versioning' 'Directory.Build.props reports v0.8.18.0' -Severity Critical

Test-MystTiqTextMatch $ctx 'src\PalworldManager\app.manifest' `
    'assemblyIdentity version="0\.8\.18\.0"' `
    'Versioning' 'app.manifest reports v0.8.18.0' -Severity Critical
...
Test-MystTiqFile $ctx 'scripts\Test-v0.8.17.0-Logic.ps1' 'Regression' 'v0.8.17.0 logic gate remains available' -Severity High
...
# ---------------------------------------------------------------------------
# 3. v0.8.18.0 Contracts -- bandwidth (the server's Engine.ini network limits)
# ---------------------------------------------------------------------------
$netPolicy = Get-MystTiqText $ctx 'src\MystTiq.Core\Services\ServerNetworkPolicy.cs'
Add-MystTiqCheck $ctx 'v0.8.18.0 Contracts' 'the policy: game defaults or custom limits (0.25..100 Mbit/s per player, 10..120 updates), the game''s real defaults (64 Mbit/s, 60 updates, 20 on Linux), the planner with 80 % headroom' `
...
Add-MystTiqCheck $ctx 'v0.8.18.0 Contracts' 'Engine.ini edits touch only MaxClientRate, MaxInternetClientRate and NetServerMaxTickRate in [/Script/OnlineSubsystemUtils.IpNetDriver] (any case), keep every other line and the line endings, and keep the original once' `
...
Add-MystTiqCheck $ctx 'v0.8.18.0 Contracts' 'the limits are written before every start on both platforms (every start path), never blocking it; saving while running waits for the next start; reading Viewer, saving Admin' `
...
Add-MystTiqCheck $ctx 'v0.8.18.0 Contracts' 'the HOST tab has a read-only-below-Admin bandwidth card with the estimate and suggestion; harnesses, the Windows smoke (supervisor restarts) and the Linux start path cover it' `
...
Test-MystTiqFile $ctx 'docs\architecture\v0.8.18.0-bandwidth.md' 'v0.8.18.0 Contracts' 'v0.8.18.0 architecture doc is present' -Severity Critical

# ---------------------------------------------------------------------------
# 4. Documentation
# ---------------------------------------------------------------------------
$docText = (Get-ChildItem (Join-Path $root 'docs') -Recurse -File -Include *.md | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n"
Add-MystTiqCheck $ctx 'Documentation' 'v0.8.18.0 is documented' ([regex]::IsMatch($docText, 'v0\.8\.18\.0')) -Severity High
Test-MystTiqFile $ctx 'release-notes\v0.8.18.0.md' 'Documentation' 'v0.8.18.0 release notes exist' -Severity High
$changelogText = Get-MystTiqText $ctx 'CHANGELOG.md'
Add-MystTiqCheck $ctx 'Documentation' 'CHANGELOG.md has a v0.8.18.0 entry' ($changelogText -match '## v0\.8\.18\.0') -Severity High
...
# ---------------------------------------------------------------------------
if ($RunBuild) {
    $frozenZip = if ($FrozenBaselineZip) { $FrozenBaselineZip } else { Join-Path $root '..\_Backups\MystTiqPalworldServer\v0.8.17.0\MystTiqPalworldServer_v0.8.17.0_FullSource.zip' }
    if (-not (Test-Path $frozenZip -PathType Leaf)) {
        # v0.8.1.0: reported, not fatal: the rest of the gate is still worth running on a machine without it.
        Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.17.0 checkpoint logic gate still passes' $false -Skipped -Severity Critical `
            -Details "Baseline archive not found at $frozenZip. Run on the release machine, or pass -FrozenBaselineZip <path>."
    }
    else {
    $frozenRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.17.0-frozen-baseline'
    ...
    $frozenGateLog = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.17.0-frozen-gate-output.txt'
    try {
        & (Join-Path $frozenProjectRoot 'scripts\Test-v0.8.17.0-Logic.ps1') -ProjectRoot $frozenProjectRoot *> $frozenGateLog
    } catch { }
    ...
    Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.17.0 checkpoint logic gate still passes' `
    ...
    Test-MystTiqCommand $ctx 'v0.8.18.0 Runtime' 'v0.8.18.0 bandwidth route smoke passes' {
    ...
    Test-MystTiqCommand $ctx 'v0.8.18.0 Runtime' 'MystTiq.ArtworkHarness passes (offline headless rendering, incl. the HOST tab''s bandwidth card)' {
