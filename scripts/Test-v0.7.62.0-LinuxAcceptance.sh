...
#
# Carried forward again for v0.7.62.0 (same reasoning as the v0.7.17.0 jump above -- this file had
# gone unmaintained across ~45 releases and Deploy-Test-MystTiqLinux.ps1 requires a version-matched
# copy to exist to deploy at all). SchemaVersion is still 3 (unchanged since v0.6.2.0), so the schema
# check below remains valid as-is. This pass verifies reachability/deployment health only -- it does
# NOT add coverage for the many routes shipped since v0.7.17.0 (Update Center's per-component version
# tracking, UE4SS release catalog, MOD Safe-Start diagnostic, per-tab identity color, etc.); treat
# this as "prove the current build deploys and runs cleanly on Linux," not a full endpoint audit.
VERSION="0.7.62.0"
