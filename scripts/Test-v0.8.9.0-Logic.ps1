...
$ctx = New-MystTiqTestContext -ProjectRoot $root -Version '0.8.9.0' -Suite 'Logic'

# ---------------------------------------------------------------------------
# 1. Version / build identity
# ---------------------------------------------------------------------------
Test-MystTiqTextMatch $ctx 'Directory.Build.props' `
    '<VersionPrefix>0\.8\.9\.0</VersionPrefix>' `
    'Versioning' 'Directory.Build.props reports v0.8.9.0' -Severity Critical

Test-MystTiqTextMatch $ctx 'src\PalworldManager\app.manifest' `
    'assemblyIdentity version="0\.8\.9\.0"' `
    'Versioning' 'app.manifest reports v0.8.9.0' -Severity Critical
...
Test-MystTiqFile $ctx 'scripts\Test-v0.8.8.0-Logic.ps1' 'Regression' 'v0.8.8.0 logic gate remains available' -Severity High
...
$imageRows = @([regex]::Matches($imageRecord, '\| [123] \| `(\w+)\.png` \| [^|]+ \| `([0-9A-F]{64})` \|') | ForEach-Object { [pscustomobject]@{ Name = $_.Groups[1].Value; Hash = $_.Groups[2].Value } })
...
Add-MystTiqCheck $ctx 'Regression' 'v0.8.8.0 Ribbon image icons remain: all 30 unchanged (hashes), image before vector before glyph' `
    ($imageRows.Count -eq 30 -and $imageBad.Count -eq 0 -and $xaml -match 'Text="\{Binding Glyph\}" IsVisible="\{Binding ShowGlyph\}"') `
    -Severity Critical -Details "mismatched: $($imageBad -join ', ')"

# ---------------------------------------------------------------------------
# 3. v0.8.9.0 Contracts -- Unreal crash reports in the Crash Analyzer, and an alert on a new critical finding
# ---------------------------------------------------------------------------
...
$crashTools = Get-MystTiqText $ctx 'src\MystTiq.HeadlessHost\HeadlessCrashAndSaveToolsService.cs'
$watcher = Get-MystTiqText $ctx 'src\MystTiq.HeadlessHost\HeadlessCrashReportWatcher.cs'
$observer = Get-MystTiqText $ctx 'src\MystTiq.HeadlessHost\CrashAlerts.cs'
$automationText = Get-MystTiqText $ctx 'src\MystTiq.HeadlessHost\HeadlessAutomationService.cs'

Add-MystTiqCheck $ctx 'v0.8.9.0 Contracts' 'Unreal crash contexts (FGenericCrashContext/RuntimeProperties) parse into one stamped evidence line the catalog classifies' `
    ($parser -match 'public const string ContextFileName = "CrashContext\.runtime-xml";' -and $parser -match 'document\.Root\?\.Element\("RuntimeProperties"\)' -and
     $parser -match 'Unreal crash report \{report\.Folder\}: \{report\.ErrorMessage\}') `
    -Severity Critical

$arrayAt = $catalog.IndexOf('new(new("engine-array-size"'); $fatalAt = $catalog.IndexOf('new(new("ue-fatal"')
Add-MystTiqCheck $ctx 'v0.8.9.0 Contracts' 'the TArray engine check from the real crash report has its own signature, ahead of the fatal-error catch-all, claiming only what the report states' `
    ($arrayAt -gt 0 -and $arrayAt -lt $fatalAt -and $catalog -match 'R\(@"Trying to resize TArray to an invalid size"\)' -and
     $catalog -match 'but not which part of the game hit it') `
    -Severity Critical

Add-MystTiqCheck $ctx 'v0.8.9.0 Contracts' 'the analyzer reads Pal\Saved\Crashes (newest 20), keeps report lines after the log lines, runs one analysis at a time and counts the reports' `
    ($crashTools -match 'public string CrashReportsRoot => Path\.Combine\(paths\.ServerRoot, "Pal", "Saved", "Crashes"\);' -and
     $crashTools -match 'public const int MaximumCrashReports = 20;' -and $crashTools -match 'lock \(analyzeGate\) return AnalyzeLocked\(installedModNames\);' -and
     $crashTools -match 'logEvidence\.Lines\.Concat\(reports' -and $crashTools -match 'int CrashReportsRead = 0\)') `
    -Severity Critical

Add-MystTiqCheck $ctx 'v0.8.9.0 Contracts' 'the watcher records old reports silently, waits out a mute, respects the crash-alert switch and a recent crash alert, and alerts only on findings from the new reports' `
    ($watcher -match 'CrashReportCheckOutcome\.Baseline' -and $watcher -match 'if \(AlertMutePolicy\.IsMuted\(ruleSet, now\)\)' -and
     $watcher -match 'CrashReportCheckOutcome\.Disabled' -and $watcher -match 'now - alerted < RecentCrashAlertWindow' -and
     $watcher -match 'public static IReadOnlyList<HeadlessCrashFinding> RelatedCriticalFindings' -and
     $watcher -match 'catch \(Exception ex\) when \(ex is IOException or JsonException or UnauthorizedAccessException\) \{ return null; \}') `
    -Severity Critical

Add-MystTiqCheck $ctx 'v0.8.9.0 Contracts' 'it runs on the automation tick for every profile, and the crash-recovery observer tells it when it last alerted' `
    ($automationText -match 'await crashReports\.CheckAsync\(DateTimeOffset\.UtcNow, token\);' -and
     $apiHost -match 'new HeadlessCrashReportWatcher\(paths,' -and $apiHost -match '\(\) => crashAlerts\.LastCrashDetectedUtc' -and
     $observer -match 'if \(supervisorEvent\.Kind == SupervisorEventKind\.CrashDetected\) LastCrashDetectedUtc = DateTimeOffset\.UtcNow;') `
    -Severity Critical

$harness = Get-MystTiqText $ctx 'scripts\Testing\MystTiq.LogicHarness\Program.cs'
Add-MystTiqCheck $ctx 'v0.8.9.0 Contracts' 'the logic harness covers the parser and classification, the analyzer reading reports, and every watcher case' `
    (([regex]::Matches($harness, 'RunScenario\("Crash reports: ')).Count -eq 3) -Severity Critical

$smoke = Get-MystTiqText $ctx 'scripts\Test-v0.8.9.0-RouteSmoke.ps1'
Add-MystTiqCheck $ctx 'v0.8.9.0 Contracts' 'the crash report smoke uses its own FleetRoot and synthetic reports' `
    ($smoke -match '\$cfg\.FleetRoot = \$fleetRoot' -and $smoke -match 'synthetic crash report folders') -Severity Critical
Test-MystTiqFile $ctx 'docs\architecture\v0.8.9.0-crash-reports.md' 'v0.8.9.0 Contracts' 'v0.8.9.0 architecture doc is present' -Severity Critical

# ---------------------------------------------------------------------------
# 4. Documentation
# ---------------------------------------------------------------------------
$docText = (Get-ChildItem (Join-Path $root 'docs') -Recurse -File -Include *.md | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n"
Add-MystTiqCheck $ctx 'Documentation' 'v0.8.9.0 is documented' ([regex]::IsMatch($docText, 'v0\.8\.9\.0')) -Severity High
Test-MystTiqFile $ctx 'release-notes\v0.8.9.0.md' 'Documentation' 'v0.8.9.0 release notes exist' -Severity High
$changelogText = Get-MystTiqText $ctx 'CHANGELOG.md'
Add-MystTiqCheck $ctx 'Documentation' 'CHANGELOG.md has a v0.8.9.0 entry' ($changelogText -match '## v0\.8\.9\.0') -Severity High
...
# ---------------------------------------------------------------------------
if ($RunBuild) {
    $frozenZip = if ($FrozenBaselineZip) { $FrozenBaselineZip } else { Join-Path $root '..\_Backups\MystTiqPalworldServer\v0.8.8.0\MystTiqPalworldServer_v0.8.8.0_FullSource.zip' }
    if (-not (Test-Path $frozenZip -PathType Leaf)) {
        # v0.8.1.0: reported, not fatal: the rest of the gate is still worth running on a machine without it.
        Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.8.0 checkpoint logic gate still passes' $false -Skipped -Severity Critical `
            -Details "Baseline archive not found at $frozenZip. Run on the release machine, or pass -FrozenBaselineZip <path>."
    }
    else {
    $frozenRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.8.0-frozen-baseline'
    ...
    $frozenGateLog = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.8.0-frozen-gate-output.txt'
    try {
        & (Join-Path $frozenProjectRoot 'scripts\Test-v0.8.8.0-Logic.ps1') -ProjectRoot $frozenProjectRoot *> $frozenGateLog
    } catch { }
    ...
    Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.8.0 checkpoint logic gate still passes' `
    ...
    Test-MystTiqCommand $ctx 'v0.8.9.0 Runtime' 'v0.8.9.0 crash report route smoke passes' {
    ...
    Test-MystTiqCommand $ctx 'Regression Runtime' 'MystTiq.ArtworkHarness passes (offline headless rendering, incl. the Ribbon image icons in dark and light)' {
