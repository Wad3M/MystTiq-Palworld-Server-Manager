...
#
# Carried forward again for v0.7.65.0 from the v0.7.62.0 copy (same reasoning as the v0.7.17.0 jump
# above -- Build-LinuxHeadless.ps1/Deploy-Test-MystTiqLinux.ps1 require a version-matched copy of
# this file to exist to deploy at all; v0.7.63.0/v0.7.64.0 didn't touch Linux code and so needed no
# copy of their own). SchemaVersion is still 3 (unchanged since v0.6.2.0), so the schema check below
# remains valid as-is. This pass verifies reachability/deployment health only -- it does NOT add
# coverage for the many routes shipped since v0.7.17.0; treat this as "prove the current build
# deploys and runs cleanly on Linux," not a full endpoint audit.
#
# One new targeted check WAS added for v0.7.65.0's own fix: LinuxServerLifecycleService's detached
# PalServer launch used to redirect stdout/stderr into ManagerRuntimeRoot/palserver-console.log, a
# location nothing on the read side (Live Console, HeadlessMonitoringService, Doctor) ever looked at
# -- moved to LogsRoot/MystTiq-PalServer-Console.log, matching Windows' capture exactly. That check
# only runs with --extended (it needs the live service to actually start PalServer once) and could
# NOT be run against the real reference VM this pass -- it was unreachable (ssh/ping both timed out)
# -- so it is present but genuinely unverified live; review its output the next time this script runs
# successfully against that VM before trusting it fully.
VERSION="0.7.65.0"
...
if ((EXTENDED)); then
  ...
  if [[ "$scheme" != "http" || "$auth_enabled" == "true" || "$bind" != "127.0.0.1" ]]; then
    ...
    if [[ "$initial_ready" == "true" ]]; then
      ...
      # v0.7.65.0: LinuxServerLifecycleService's detached PalServer launch used to redirect
      # stdout/stderr into ManagerRuntimeRoot/palserver-console.log, a location nothing on the read
      # side (Live Console, HeadlessMonitoringService, Doctor) ever looked at -- moved to
      # LogsRoot (ServerRoot/Pal/Saved/Logs)/MystTiq-PalServer-Console.log, matching Windows' capture
      # exactly. Derives the real ServerRoot from the live "Palworld Dedicated Server" checklist row
      # (server-environment.json, already fetched above) rather than hardcoding a path, so this works
      # against whatever install location this host is actually configured with. Confirms the file
      # exists, is non-empty, and carries this session's own MystTiq-written start marker (not just a
      # stale file left over from a previous session/version). Never verified against this real VM --
      # unreachable this pass (see header comment) -- so treat a FAIL here as high-signal, not routine.
      sleep 2
      server_exe="$(jq -r '.items[]? | select(.component=="Palworld Dedicated Server") | .location // empty' "$REPORT_ROOT/server-environment.json" 2>/dev/null)"
      if [[ -z "$server_exe" ]]; then
        record SKIP "Linux console capture lands in LogsRoot/MystTiq-PalServer-Console.log" "could not resolve ServerRoot from server-environment.json"
      else
        captured_console="$(dirname "$server_exe")/Pal/Saved/Logs/MystTiq-PalServer-Console.log"
        if [[ -s "$captured_console" ]] && grep -q "MystTiq PalServer detached console session starting" "$captured_console"; then
          record PASS "Linux console capture lands in LogsRoot/MystTiq-PalServer-Console.log" "$captured_console"
        else
          record FAIL "Linux console capture lands in LogsRoot/MystTiq-PalServer-Console.log" "not found/empty/no start marker at $captured_console"
        fi
      fi
