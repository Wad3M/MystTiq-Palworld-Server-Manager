...
$temp=Join-Path $root ("artifacts\runtime-smoke\v0.4.5.1-" + [guid]::NewGuid().ToString('N'))
...
$proc=$null
try{
    $args=@('api-run','--config',$config,'--bind-address','127.0.0.1','--api-port',"$Port",'--server-root',$missingServer,'--steamcmd',$steamCmd,'--backup-root',$backupRoot,'--runtime-root',$runtime)
