...
$parityMatrix=Text 'docs\GUI_PARITY_v0.4.6.4.md'
...
Check 'Versioning' 'Version is v0.4.6.4' ($props -match '<VersionPrefix>0\.4\.6\.4</VersionPrefix>' -and $vm -match 'Version => "v0\.4\.6\.4"')
Check 'PowerShell Safety' 'Current logic harness does not assign to reserved automatic Host variable' ((Text 'scripts\Test-v0.4.6.4-Logic.ps1') -notmatch '(?im)^\s*\$host\s*=')
...
Check 'Lifecycle Window Policy' 'Windows lifecycle reapplies proven v0.2 post-launch ShowWindow(SW_HIDE) policy to wrapper/child consoles' ($windowsLifecycle -match 'ApplyPostLaunchWindowPolicyAsync' -and $windowsLifecycle -match 'ShowWindow\(handle, SwHide\)' -and $windowsLifecycle -match 'DllImport\("user32.dll"\)' -and $windowsLifecycle -match 'for \(var attempt = 0; attempt < 40; attempt\+\+\)')
Check 'Dashboard Parity' 'Dashboard restores dense live v0.2-inspired world pulse health resource history activity log and online player surfaces' ($ui -match 'WORLD PULSE|DashboardWorldPulseText' -and $ui -match 'RESOURCE HISTORY' -and $ui -match 'LIVE ACTIVITY' -and $ui -match 'LIVE SERVER / MANAGER LOG' -and $ui -match 'ONLINE PLAYERS' -and $ui -match 'OVERALL HEALTH' -and $vm -match 'DashboardActivityLines' -and $vm -match 'MetricHistory')
...
Check 'RunBuild Gate' '-RunBuild compiles shared desktop and headless targets for Windows and Linux' ($MyInvocation.MyCommand.Path -and (Text 'scripts\Test-v0.4.6.4-Logic.ps1') -match '-Action WindowsHeadless' -and (Text 'scripts\Test-v0.4.6.4-Logic.ps1') -match '-Action LinuxHeadless' -and (Text 'scripts\Test-v0.4.6.4-Logic.ps1') -match '-Action DesktopWindows' -and (Text 'scripts\Test-v0.4.6.4-Logic.ps1') -match '-Action DesktopLinux')
Check 'Documentation' 'README identifies v0.4.6.4 candidate and v0.4.5.1 baseline' ($readme -match 'Current release candidate: v0\.4\.6\.4' -and $readme -match 'Official baseline: v0\.4\.5\.1')
Check 'Documentation' 'Roadmap identifies v0.4.6.4 current candidate' ($road -match 'v0\.4\.6\.4.+Current Release Candidate')


Check 'Packaging' 'Current-version logic test exists at the exact release path' (Test-Path (Join-Path $root 'scripts\Test-v0.4.6.4-Logic.ps1') -PathType Leaf)
...
Check 'Linux Packaging' 'Current Linux acceptance and production-readiness scripts exist' ((Test-Path (Join-Path $root 'scripts\Test-v0.4.6.4-LinuxAcceptance.sh') -PathType Leaf) -and (Test-Path (Join-Path $root 'scripts\Test-v0.4.6.4-ProductionReadiness.sh') -PathType Leaf))
...
Check 'Testing UX' 'README includes full clean/unblock/validate/current-logic command sequence' ($readme -match 'Get-ChildItem \. -Recurse -Filter \*\.ps1 \| Unblock-File' -and $readme -match 'Test-v0\.4\.6\.4-Logic\.ps1')
...
Check 'Runtime Smoke' 'Windows API runtime smoke script exists and checks health poll config and distribution endpoints' ((Test-Path (Join-Path $root 'scripts\Test-v0.4.6.4-RuntimeSmoke.ps1') -PathType Leaf) -and (Text 'scripts\Test-v0.4.6.4-RuntimeSmoke.ps1') -match '/healthz' -and (Text 'scripts\Test-v0.4.6.4-RuntimeSmoke.ps1') -match '/api/v1/status/poll' -and (Text 'scripts\Test-v0.4.6.4-RuntimeSmoke.ps1') -match '/api/v1/config/editable')
...
Check 'Runtime Smoke' 'Isolated start-route smoke cannot launch the user PalServer and verifies a safe 424 response' ((Text 'scripts\Test-v0.4.6.4-RuntimeSmoke.ps1') -match 'missing-server' -and (Text 'scripts\Test-v0.4.6.4-RuntimeSmoke.ps1') -match '/api/v1/server/start' -and (Text 'scripts\Test-v0.4.6.4-RuntimeSmoke.ps1') -match 'StatusCode -ne 424')
...
Check 'Runtime Smoke' 'Runtime smoke exercises player capability gating and persistent audit log' ((Text 'scripts\Test-v0.4.6.4-RuntimeSmoke.ps1') -match 'Player administration route is live' -and (Text 'scripts\Test-v0.4.6.4-RuntimeSmoke.ps1') -match 'Persistent activity/audit log captures management events')
...
Check 'Dashboard UX' 'CPU presentation is a live history graph instead of a single progress bar' ($ui -match 'ItemsSource="\{Binding MetricHistory\}"' -and $ui -match 'CpuGraphHeight' -and $monitoringDtos -match 'CpuGraphHeight' -and $ui -notmatch 'Value="\{Binding CpuPercentValue\}" Height="5"')
...
Check 'World Pulse' 'Aggregate status poll carries authoritative world clock/save, backup and Palworld identity support data' ($apiHostSource -match 'ExploreDashboard\(\)' -and $apiHostSource -match 'backupInventory' -and $apiHostSource -match 'palworldSettings' -and $pollDto -match 'WorldExplorerSnapshotDto World' -and $pollDto -match 'BackupInventoryDto BackupInventory' -and $pollDto -match 'PalworldConfigurationSnapshotDto PalworldSettings' -and $worldExplorerService -match 'GameDateTimeTicks')
...
Check 'Server Setup Wiring' 'Environment checklist flows View -> ViewModel -> API -> endpoint -> headless platform inspection' ($vm -match 'RefreshEnvironmentAsync' -and $api -match 'GetEnvironmentChecklistAsync' -and $client -match '/api/v1/server/environment' -and $apiHostSource -match '/api/v1/server/environment' -and $environmentService -match 'Steam Client' -and $environmentService -match 'Palworld Dedicated Server' -and $environmentService -match 'Backup Storage')
Check 'Server Setup Behavior' 'Install missing can provision SteamCMD before Palworld distribution update' ((Text 'src\MystTiq.HeadlessHost\HeadlessServerDistributionService.cs') -match 'ProvisionSteamCmdAsync' -and (Text 'src\MystTiq.HeadlessHost\HeadlessServerDistributionService.cs') -match 'SteamCmdPackageUri' -and (Text 'src\MystTiq.HeadlessHost\HeadlessServerDistributionService.cs') -match 'ExtractSteamCmdPackage')
Check 'Navigation Selection' 'Every navigation destination binds selected-state highlight to ViewModel page state' ($ui -match 'Classes\.active="\{Binding IsDashboardPage\}"' -and $ui -match 'Classes\.active="\{Binding IsServerSetupPage\}"' -and $ui -match 'Classes\.active="\{Binding IsPlayersPage\}"' -and $ui -match 'Classes\.active="\{Binding IsSettingsPage\}"' -and $appXaml -match 'Button\.navitem\.active')
...
if($RunBuild){
    ...
    & (Join-Path $root 'scripts\Test-v0.4.6.4-RuntimeSmoke.ps1') -ProjectRoot $root
}
$failed=@($checks|Where-Object{-not $_.Passed})
Write-Host "`n================ MystTiq v0.4.6.4 Summary ================"
Write-Host "Passed: $($checks.Count-$failed.Count) / $($checks.Count)"
if($ExportJson){$dir=Join-Path $root 'artifacts\logic-tests';New-Item $dir -ItemType Directory -Force|Out-Null;$path=Join-Path $dir "MystTiq_v0.4.6.4_$(Get-Date -Format yyyyMMdd-HHmmss).json";$checks|ConvertTo-Json -Depth 4|Set-Content $path;Write-Host "JSON report: $path"}
if($failed.Count){$failed|Format-Table -AutoSize;throw "v0.4.6.4 logic gate failed: $($failed.Count) check(s)."}
Write-Host 'v0.4.6.4 logic gate passed.' -ForegroundColor Green
