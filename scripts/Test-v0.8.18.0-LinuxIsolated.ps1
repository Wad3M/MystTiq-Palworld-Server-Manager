...
# v0.8.18.0: an isolated MystTiq on the Linux test VM for bandwidth (Engine.ini network limits written on the real Linux start path). It publishes
# the linux-x64 headless, copies it with Test-v0.8.18.0-LinuxIsolated.sh to a temp folder on the VM, runs the check
# there (own config, FleetRoot, runtime and port 18419, a stand-in server started by MystTiq through a fake PalServer.sh; the
...
$work = Join-Path $root ("artifacts\linux-isolated\v0.8.18.0-" + [guid]::NewGuid().ToString('N'))
...
$sh = [IO.File]::ReadAllText((Join-Path $root 'scripts\Test-v0.8.18.0-LinuxIsolated.sh')).Replace("`r`n", "`n")
...
$dir = '/tmp/mysttiq-v0818-isolated'
...
if (-not ($lines -match '^exit=0$')) { throw 'MystTiq v0.8.18.0 Linux isolated check failed.' }
Write-Host 'MystTiq v0.8.18.0 Linux isolated check passed.' -ForegroundColor Green
