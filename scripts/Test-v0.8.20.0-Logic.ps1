...
$ctx = New-MystTiqTestContext -ProjectRoot $root -Version '0.8.20.0' -Suite 'Logic'

# ---------------------------------------------------------------------------
# 1. Version / build identity
# ---------------------------------------------------------------------------
Test-MystTiqTextMatch $ctx 'Directory.Build.props' `
    '<VersionPrefix>0\.8\.20\.0</VersionPrefix>' `
    'Versioning' 'Directory.Build.props reports v0.8.20.0' -Severity Critical

Test-MystTiqTextMatch $ctx 'src\PalworldManager\app.manifest' `
    'assemblyIdentity version="0\.8\.20\.0"' `
    'Versioning' 'app.manifest reports v0.8.20.0' -Severity Critical
...
Test-MystTiqFile $ctx 'scripts\Test-v0.8.19.0-Logic.ps1' 'Regression' 'v0.8.19.0 logic gate remains available' -Severity High
...
# ---------------------------------------------------------------------------
# 3. v0.8.20.0 Contracts -- host history on the HOST tab
# ---------------------------------------------------------------------------
...
Add-MystTiqCheck $ctx 'v0.8.20.0 Contracts' 'one reading a minute (the first at start), kept 7 days in the fleet folder, the busiest adapter only (never added up), thinned by averaging with peaks from every reading' `
...
Add-MystTiqCheck $ctx 'v0.8.20.0 Contracts' 'the HOST tab shows the history (hour, day, week) with its summary; a reading without a value is a gap, not a zero; it reloads about once a minute' `
...
Add-MystTiqCheck $ctx 'v0.8.20.0 Contracts' 'the harnesses, the Windows smoke (a reading at start, the fleet folder, a restart) and the Linux check (/proc) cover it' `
...
Test-MystTiqFile $ctx 'docs\architecture\v0.8.20.0-host-history.md' 'v0.8.20.0 Contracts' 'v0.8.20.0 architecture doc is present' -Severity Critical

# ---------------------------------------------------------------------------
# 4. Documentation
# ---------------------------------------------------------------------------
$docText = (Get-ChildItem (Join-Path $root 'docs') -Recurse -File -Include *.md | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n"
Add-MystTiqCheck $ctx 'Documentation' 'v0.8.20.0 is documented' ([regex]::IsMatch($docText, 'v0\.8\.20\.0')) -Severity High
Test-MystTiqFile $ctx 'release-notes\v0.8.20.0.md' 'Documentation' 'v0.8.20.0 release notes exist' -Severity High
$changelogText = Get-MystTiqText $ctx 'CHANGELOG.md'
Add-MystTiqCheck $ctx 'Documentation' 'CHANGELOG.md has a v0.8.20.0 entry' ($changelogText -match '## v0\.8\.20\.0') -Severity High
...
# ---------------------------------------------------------------------------
if ($RunBuild) {
    $frozenZip = if ($FrozenBaselineZip) { $FrozenBaselineZip } else { Join-Path $root '..\_Backups\MystTiqPalworldServer\v0.8.19.0\MystTiqPalworldServer_v0.8.19.0_FullSource.zip' }
    if (-not (Test-Path $frozenZip -PathType Leaf)) {
        # v0.8.1.0: reported, not fatal: the rest of the gate is still worth running on a machine without it.
        Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.19.0 checkpoint logic gate still passes' $false -Skipped -Severity Critical `
            -Details "Baseline archive not found at $frozenZip. Run on the release machine, or pass -FrozenBaselineZip <path>."
    }
    else {
    $frozenRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.19.0-frozen-baseline'
    ...
    $frozenGateLog = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.19.0-frozen-gate-output.txt'
    try {
        & (Join-Path $frozenProjectRoot 'scripts\Test-v0.8.19.0-Logic.ps1') -ProjectRoot $frozenProjectRoot *> $frozenGateLog
    } catch { }
    ...
    Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.19.0 checkpoint logic gate still passes' `
    ...
    Test-MystTiqCommand $ctx 'v0.8.20.0 Runtime' 'v0.8.20.0 host history smoke passes' {
    ...
    Test-MystTiqCommand $ctx 'v0.8.20.0 Runtime' 'MystTiq.ArtworkHarness passes (offline headless rendering, incl. the HOST tab''s history)' {
