...
Check 'Versioning' 'Version is v0.4.5.0' ($props -match '<VersionPrefix>0\.4\.5\.0</VersionPrefix>' -and $vm -match 'Version => "v0\.4\.5\.0"')
Check 'PowerShell Safety' 'Current logic harness does not assign to reserved automatic Host variable' ((Text 'scripts\Test-v0.4.5.0-Logic.ps1') -notmatch '(?im)^\s*\$host\s*=')
...
Check 'RunBuild Gate' '-RunBuild compiles shared desktop and headless targets for Windows and Linux' ($MyInvocation.MyCommand.Path -and (Text 'scripts\Test-v0.4.5.0-Logic.ps1') -match '-Action WindowsHeadless' -and (Text 'scripts\Test-v0.4.5.0-Logic.ps1') -match '-Action LinuxHeadless' -and (Text 'scripts\Test-v0.4.5.0-Logic.ps1') -match '-Action DesktopWindows' -and (Text 'scripts\Test-v0.4.5.0-Logic.ps1') -match '-Action DesktopLinux')
Check 'Documentation' 'README identifies v0.4.5.0 candidate and v0.4.4.3 baseline' ($readme -match 'v0\.4\.5\.0' -and $readme -match 'Official baseline: v0\.4\.4\.3')
Check 'Documentation' 'Roadmap identifies v0.4.5.0 current candidate' ($road -match 'v0\.4\.5\.0.+Current Release Candidate')


Check 'Packaging' 'Current-version logic test exists at the exact release path' (Test-Path (Join-Path $root 'scripts\Test-v0.4.5.0-Logic.ps1') -PathType Leaf)
...
Check 'Linux Packaging' 'Current Linux acceptance and production-readiness scripts exist' ((Test-Path (Join-Path $root 'scripts\Test-v0.4.5.0-LinuxAcceptance.sh') -PathType Leaf) -and (Test-Path (Join-Path $root 'scripts\Test-v0.4.5.0-ProductionReadiness.sh') -PathType Leaf))
...
Check 'Testing UX' 'README includes full clean/unblock/validate/current-logic command sequence' ($readme -match 'Get-ChildItem \. -Recurse -Filter \*\.ps1 \| Unblock-File' -and $readme -match 'Test-v0\.4\.5\.0-Logic\.ps1')
...
Check 'Runtime Smoke' 'Windows API runtime smoke script exists and checks health poll config and distribution endpoints' ((Test-Path (Join-Path $root 'scripts\Test-v0.4.5.0-RuntimeSmoke.ps1') -PathType Leaf) -and (Text 'scripts\Test-v0.4.5.0-RuntimeSmoke.ps1') -match '/healthz' -and (Text 'scripts\Test-v0.4.5.0-RuntimeSmoke.ps1') -match '/api/v1/status/poll' -and (Text 'scripts\Test-v0.4.5.0-RuntimeSmoke.ps1') -match '/api/v1/config/editable')
...
if($RunBuild){
    ...
    & (Join-Path $root 'scripts\Test-v0.4.5.0-RuntimeSmoke.ps1') -ProjectRoot $root
}
$failed=@($checks|Where-Object{-not $_.Passed})
Write-Host "`n================ MystTiq v0.4.5.0 Summary ================"
Write-Host "Passed: $($checks.Count-$failed.Count) / $($checks.Count)"
if($ExportJson){$dir=Join-Path $root 'artifacts\logic-tests';New-Item $dir -ItemType Directory -Force|Out-Null;$path=Join-Path $dir "MystTiq_v0.4.5.0_$(Get-Date -Format yyyyMMdd-HHmmss).json";$checks|ConvertTo-Json -Depth 4|Set-Content $path;Write-Host "JSON report: $path"}
if($failed.Count){$failed|Format-Table -AutoSize;throw "v0.4.5.0 logic gate failed: $($failed.Count) check(s)."}
Write-Host 'v0.4.5.0 logic gate passed.' -ForegroundColor Green
