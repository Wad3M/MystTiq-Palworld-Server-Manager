...
$parityMatrix=Text 'docs\GUI_PARITY_v0.4.5.1.md'


Check 'Versioning' 'Version is v0.4.5.1' ($props -match '<VersionPrefix>0\.4\.5\.1</VersionPrefix>' -and $vm -match 'Version => "v0\.4\.5\.1"')
Check 'PowerShell Safety' 'Current logic harness does not assign to reserved automatic Host variable' ((Text 'scripts\Test-v0.4.5.1-Logic.ps1') -notmatch '(?im)^\s*\$host\s*=')
...
Check 'RunBuild Gate' '-RunBuild compiles shared desktop and headless targets for Windows and Linux' ($MyInvocation.MyCommand.Path -and (Text 'scripts\Test-v0.4.5.1-Logic.ps1') -match '-Action WindowsHeadless' -and (Text 'scripts\Test-v0.4.5.1-Logic.ps1') -match '-Action LinuxHeadless' -and (Text 'scripts\Test-v0.4.5.1-Logic.ps1') -match '-Action DesktopWindows' -and (Text 'scripts\Test-v0.4.5.1-Logic.ps1') -match '-Action DesktopLinux')
Check 'Documentation' 'README identifies v0.4.5.1 candidate and v0.4.4.3 baseline' ($readme -match 'Current release candidate: v0\.4\.5\.1' -and $readme -match 'Official baseline: v0\.4\.4\.3')
Check 'Documentation' 'Roadmap identifies v0.4.5.1 current candidate' ($road -match 'v0\.4\.5\.1.+Current Release Candidate')


Check 'Packaging' 'Current-version logic test exists at the exact release path' (Test-Path (Join-Path $root 'scripts\Test-v0.4.5.1-Logic.ps1') -PathType Leaf)
...
Check 'Linux Packaging' 'Current Linux acceptance and production-readiness scripts exist' ((Test-Path (Join-Path $root 'scripts\Test-v0.4.5.1-LinuxAcceptance.sh') -PathType Leaf) -and (Test-Path (Join-Path $root 'scripts\Test-v0.4.5.1-ProductionReadiness.sh') -PathType Leaf))
...
Check 'Testing UX' 'README includes full clean/unblock/validate/current-logic command sequence' ($readme -match 'Get-ChildItem \. -Recurse -Filter \*\.ps1 \| Unblock-File' -and $readme -match 'Test-v0\.4\.5\.1-Logic\.ps1')
...
Check 'Runtime Smoke' 'Windows API runtime smoke script exists and checks health poll config and distribution endpoints' ((Test-Path (Join-Path $root 'scripts\Test-v0.4.5.1-RuntimeSmoke.ps1') -PathType Leaf) -and (Text 'scripts\Test-v0.4.5.1-RuntimeSmoke.ps1') -match '/healthz' -and (Text 'scripts\Test-v0.4.5.1-RuntimeSmoke.ps1') -match '/api/v1/status/poll' -and (Text 'scripts\Test-v0.4.5.1-RuntimeSmoke.ps1') -match '/api/v1/config/editable')
...
Check 'Runtime Smoke' 'Isolated start-route smoke cannot launch the user PalServer and verifies a safe 424 response' ((Text 'scripts\Test-v0.4.5.1-RuntimeSmoke.ps1') -match 'missing-server' -and (Text 'scripts\Test-v0.4.5.1-RuntimeSmoke.ps1') -match '/api/v1/server/start' -and (Text 'scripts\Test-v0.4.5.1-RuntimeSmoke.ps1') -match 'StatusCode -ne 424')
...
Check 'Runtime Smoke' 'Runtime smoke exercises player capability gating and persistent audit log' ((Text 'scripts\Test-v0.4.5.1-RuntimeSmoke.ps1') -match 'Player administration route is live' -and (Text 'scripts\Test-v0.4.5.1-RuntimeSmoke.ps1') -match 'Persistent activity/audit log captures management events')
...
if($RunBuild){
    ...
    & (Join-Path $root 'scripts\Test-v0.4.5.1-RuntimeSmoke.ps1') -ProjectRoot $root
}
$failed=@($checks|Where-Object{-not $_.Passed})
Write-Host "`n================ MystTiq v0.4.5.1 Summary ================"
Write-Host "Passed: $($checks.Count-$failed.Count) / $($checks.Count)"
if($ExportJson){$dir=Join-Path $root 'artifacts\logic-tests';New-Item $dir -ItemType Directory -Force|Out-Null;$path=Join-Path $dir "MystTiq_v0.4.5.1_$(Get-Date -Format yyyyMMdd-HHmmss).json";$checks|ConvertTo-Json -Depth 4|Set-Content $path;Write-Host "JSON report: $path"}
if($failed.Count){$failed|Format-Table -AutoSize;throw "v0.4.5.1 logic gate failed: $($failed.Count) check(s)."}
Write-Host 'v0.4.5.1 logic gate passed.' -ForegroundColor Green
