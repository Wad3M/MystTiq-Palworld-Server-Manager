#!/usr/bin/env bash
set -uo pipefail
# Carried forward from Test-v0.7.17.0-ProductionReadiness.sh for the same reason documented at the
# top of Test-v0.7.62.0-LinuxAcceptance.sh -- Build-LinuxHeadless.ps1 requires a version-matched copy
# of this script to exist to build the Linux archive at all. Not a full re-audit of every route added
# since v0.7.17.0; verifies deployment/production-readiness health only.
VERSION="0.7.62.0"
