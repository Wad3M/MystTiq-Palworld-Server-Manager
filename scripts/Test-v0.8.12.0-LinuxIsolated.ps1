...
# v0.8.12.0: an isolated MystTiq on the Linux test VM, exercising what v0.8.9.0-v0.8.12.0 added. It publishes the
# linux-x64 headless, builds a synthetic server folder (a crash report, a world save with Pals, Linux settings), copies
# both to a temp folder on the VM, runs Test-v0.8.12.0-LinuxIsolated.sh there (own config, FleetRoot, runtime and port
...
$work = Join-Path $root ("artifacts\linux-isolated\v0.8.12.0-" + [guid]::NewGuid().ToString('N'))
...
$sh = [IO.File]::ReadAllText((Join-Path $root 'scripts\Test-v0.8.12.0-LinuxIsolated.sh')).Replace("`r`n", "`n")
[IO.File]::WriteAllText((Join-Path $work 'check.sh'), $sh)
$tgz = Join-Path $work 'linux-isolated.tgz'
Push-Location $work; tar -czf $tgz app/mysttiq-server fixture check.sh; Pop-Location
...
$dir = '/tmp/mysttiq-v0812-isolated'
...
if (-not ($lines -match '^exit=0$')) { throw 'MystTiq v0.8.12.0 Linux isolated check failed.' }
Write-Host 'MystTiq v0.8.12.0 Linux isolated check passed.' -ForegroundColor Green
