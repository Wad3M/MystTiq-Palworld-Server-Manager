...
$ctx = New-MystTiqTestContext -ProjectRoot $root -Version '0.8.22.0' -Suite 'Logic'

# ---------------------------------------------------------------------------
# 1. Version / build identity
# ---------------------------------------------------------------------------
Test-MystTiqTextMatch $ctx 'Directory.Build.props' `
    '<VersionPrefix>0\.8\.22\.0</VersionPrefix>' `
    'Versioning' 'Directory.Build.props reports v0.8.22.0' -Severity Critical

Test-MystTiqTextMatch $ctx 'src\PalworldManager\app.manifest' `
    'assemblyIdentity version="0\.8\.22\.0"' `
    'Versioning' 'app.manifest reports v0.8.22.0' -Severity Critical
...
Test-MystTiqFile $ctx 'scripts\Test-v0.8.21.0-Logic.ps1' 'Regression' 'v0.8.21.0 logic gate remains available' -Severity High
...
$gateSelf821 = Get-MystTiqText $ctx 'scripts\Test-v0.8.22.0-Logic.ps1'
...
# ---------------------------------------------------------------------------
# 3. v0.8.22.0 Contracts -- the remaining sign-in tests: Owner, the Ribbon gating, the Desktop on Linux
# ---------------------------------------------------------------------------
...
Add-MystTiqCheck $ctx 'v0.8.22.0 Contracts' 'the remote sign-in covers all four roles (Owner too), the Ribbon gating on every page in the signed-in window, and the server agreeing with each kind of gated button' `
...
Add-MystTiqCheck $ctx 'v0.8.22.0 Contracts' 'the Desktop itself runs on Linux: the harness is published for linux-x64 and run on the VM with its home in /tmp, the VM user''s settings checked untouched, and everything torn down' `
...
Add-MystTiqCheck $ctx 'v0.8.22.0 Contracts' 'the window controls, settings and tab close icons are drawn (no Segoe Fluent Icons font, which Linux and Windows 10 lack), and the harness checks it' `
...
Test-MystTiqFile $ctx 'docs\architecture\v0.8.22.0-sign-in-tests.md' 'v0.8.22.0 Contracts' 'v0.8.22.0 architecture doc is present' -Severity Critical

# ---------------------------------------------------------------------------
# 4. Documentation
# ---------------------------------------------------------------------------
$docText = (Get-ChildItem (Join-Path $root 'docs') -Recurse -File -Include *.md | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n"
Add-MystTiqCheck $ctx 'Documentation' 'v0.8.22.0 is documented' ([regex]::IsMatch($docText, 'v0\.8\.22\.0')) -Severity High
Test-MystTiqFile $ctx 'release-notes\v0.8.22.0.md' 'Documentation' 'v0.8.22.0 release notes exist' -Severity High
$changelogText = Get-MystTiqText $ctx 'CHANGELOG.md'
Add-MystTiqCheck $ctx 'Documentation' 'CHANGELOG.md has a v0.8.22.0 entry' ($changelogText -match '## v0\.8\.22\.0') -Severity High
...
# ---------------------------------------------------------------------------
if ($RunBuild) {
    $frozenZip = if ($FrozenBaselineZip) { $FrozenBaselineZip } else { Join-Path $root '..\_Backups\MystTiqPalworldServer\v0.8.21.0\MystTiqPalworldServer_v0.8.21.0_FullSource.zip' }
    if (-not (Test-Path $frozenZip -PathType Leaf)) {
        # v0.8.1.0: reported, not fatal: the rest of the gate is still worth running on a machine without it.
        Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.21.0 checkpoint logic gate still passes' $false -Skipped -Severity Critical `
            -Details "Baseline archive not found at $frozenZip. Run on the release machine, or pass -FrozenBaselineZip <path>."
    }
    else {
    $frozenRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.21.0-frozen-baseline'
    ...
    $frozenGateLog = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.21.0-frozen-gate-output.txt'
    try {
        & (Join-Path $frozenProjectRoot 'scripts\Test-v0.8.21.0-Logic.ps1') -ProjectRoot $frozenProjectRoot *> $frozenGateLog
    } catch { }
    ...
    Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.21.0 checkpoint logic gate still passes' `
    ...
    if ($reachable) {
        ...
        Test-MystTiqCommand $ctx 'v0.8.22.0 Runtime' 'the Desktop signs in to a real remote MystTiq as Viewer, Operator, Admin and Owner, from Windows and from Linux' {
        ...
        Add-MystTiqCheck $ctx 'v0.8.22.0 Runtime' 'the Desktop signs in to a real remote MystTiq as Viewer, Operator, Admin and Owner, from Windows and from Linux' $false -Skipped -Severity High `
