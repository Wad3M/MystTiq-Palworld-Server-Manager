...
$ctx = New-MystTiqTestContext -ProjectRoot $root -Version '0.8.19.0' -Suite 'Logic'

# ---------------------------------------------------------------------------
# 1. Version / build identity
# ---------------------------------------------------------------------------
Test-MystTiqTextMatch $ctx 'Directory.Build.props' `
    '<VersionPrefix>0\.8\.19\.0</VersionPrefix>' `
    'Versioning' 'Directory.Build.props reports v0.8.19.0' -Severity Critical

Test-MystTiqTextMatch $ctx 'src\PalworldManager\app.manifest' `
    'assemblyIdentity version="0\.8\.19\.0"' `
    'Versioning' 'app.manifest reports v0.8.19.0' -Severity Critical
...
Test-MystTiqFile $ctx 'scripts\Test-v0.8.18.0-Logic.ps1' 'Regression' 'v0.8.18.0 logic gate remains available' -Severity High
...
# ---------------------------------------------------------------------------
# 3. v0.8.19.0 Contracts -- every route declares its role; the Ribbon follows it
# ---------------------------------------------------------------------------
...
Add-MystTiqCheck $ctx 'v0.8.19.0 Contracts' 'secure by default: RequireRole records its role, and each server''s route group refuses a route that declares none below Viewer (reading) or Admin (changing), scoped to that server' `
...
Add-MystTiqCheck $ctx 'v0.8.19.0 Contracts' 'no fleet-level route is left without a role: only sign-in, sign-out, password change, whoami and the health check' `
    ($extra.Count -eq 0 -and @($unguarded).Count -eq $exempt.Count) -Severity Critical -Details ($(if ($extra.Count) { "Unguarded: $($extra -join ', ')" } else { '' }))

Add-MystTiqCheck $ctx 'v0.8.19.0 Contracts' 'the Operator-level changes say so (save now, notifications, backup checks, crash analysis, update), everything else changing needs Admin: RCON commands, kick/ban, PalWorldSettings.ini, mods' `
...
$ribbonModel = Get-MystTiqText $ctx 'src\MystTiq.Desktop\Models\RibbonActionViewModel.cs'
Add-MystTiqCheck $ctx 'v0.8.19.0 Contracts' 'the Ribbon follows the signed-in role (disabled with the role in its tooltip), rebuilt when the role changes; kick, ban, teleport and RCON buttons need Admin' `
...
Add-MystTiqCheck $ctx 'v0.8.19.0 Contracts' 'the harnesses and the smoke (authentication on: Viewer, Operator, Admin and another server''s account) cover it' `
...
Test-MystTiqFile $ctx 'docs\architecture\v0.8.19.0-route-roles.md' 'v0.8.19.0 Contracts' 'v0.8.19.0 architecture doc is present' -Severity Critical

# ---------------------------------------------------------------------------
# 4. Documentation
# ---------------------------------------------------------------------------
$docText = (Get-ChildItem (Join-Path $root 'docs') -Recurse -File -Include *.md | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n"
Add-MystTiqCheck $ctx 'Documentation' 'v0.8.19.0 is documented' ([regex]::IsMatch($docText, 'v0\.8\.19\.0')) -Severity High
Test-MystTiqFile $ctx 'release-notes\v0.8.19.0.md' 'Documentation' 'v0.8.19.0 release notes exist' -Severity High
$changelogText = Get-MystTiqText $ctx 'CHANGELOG.md'
Add-MystTiqCheck $ctx 'Documentation' 'CHANGELOG.md has a v0.8.19.0 entry' ($changelogText -match '## v0\.8\.19\.0') -Severity High
...
# ---------------------------------------------------------------------------
if ($RunBuild) {
    $frozenZip = if ($FrozenBaselineZip) { $FrozenBaselineZip } else { Join-Path $root '..\_Backups\MystTiqPalworldServer\v0.8.18.0\MystTiqPalworldServer_v0.8.18.0_FullSource.zip' }
    if (-not (Test-Path $frozenZip -PathType Leaf)) {
        # v0.8.1.0: reported, not fatal: the rest of the gate is still worth running on a machine without it.
        Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.18.0 checkpoint logic gate still passes' $false -Skipped -Severity Critical `
            -Details "Baseline archive not found at $frozenZip. Run on the release machine, or pass -FrozenBaselineZip <path>."
    }
    else {
    $frozenRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.18.0-frozen-baseline'
    ...
    $frozenGateLog = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.18.0-frozen-gate-output.txt'
    try {
        & (Join-Path $frozenProjectRoot 'scripts\Test-v0.8.18.0-Logic.ps1') -ProjectRoot $frozenProjectRoot *> $frozenGateLog
    } catch { }
    ...
    Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.18.0 checkpoint logic gate still passes' `
    ...
    Test-MystTiqCommand $ctx 'v0.8.19.0 Runtime' 'v0.8.19.0 route roles smoke passes' {
    ...
    Test-MystTiqCommand $ctx 'v0.8.19.0 Runtime' 'MystTiq.ArtworkHarness passes (offline headless rendering, incl. the Ribbon for every role)' {
