...
$ctx = New-MystTiqTestContext -ProjectRoot $root -Version '0.8.23.0' -Suite 'Logic'

# ---------------------------------------------------------------------------
# 1. Version / build identity
# ---------------------------------------------------------------------------
Test-MystTiqTextMatch $ctx 'Directory.Build.props' `
    '<VersionPrefix>0\.8\.23\.0</VersionPrefix>' `
    'Versioning' 'Directory.Build.props reports v0.8.23.0' -Severity Critical

Test-MystTiqTextMatch $ctx 'src\PalworldManager\app.manifest' `
    'assemblyIdentity version="0\.8\.23\.0"' `
    'Versioning' 'app.manifest reports v0.8.23.0' -Severity Critical
...
Test-MystTiqFile $ctx 'scripts\Test-v0.8.22.0-Logic.ps1' 'Regression' 'v0.8.22.0 logic gate remains available' -Severity High
...
$gateSelf821 = Get-MystTiqText $ctx 'scripts\Test-v0.8.23.0-Logic.ps1'
...
# ---------------------------------------------------------------------------
# 3. v0.8.23.0 Contracts -- every button follows the role
# ---------------------------------------------------------------------------
...
Add-MystTiqCheck $ctx 'v0.8.23.0 Contracts' 'the command classes take a role gate that decides whether they can run; it is installed once every command exists and re-evaluated when the role changes; the Ribbon uses the same table' `
...
Add-MystTiqCheck $ctx 'v0.8.23.0 Contracts' 'the role table matches the code: every command reaching a route above Viewer is listed with that route''s role, and nothing else is listed' `
    ($derived823.Count -ge 100 -and $tableWrong823.Count -eq 0) -Severity Critical -Details ($tableWrong823 -join ' / ')
Add-MystTiqCheck $ctx 'v0.8.23.0 Contracts' 'every API client change is matched to a route (so no command''s role is guessed)' ($unresolved823.Count -eq 0) -Severity Critical -Details ($unresolved823 -join ' / ')
...
Add-MystTiqCheck $ctx 'v0.8.23.0 Contracts' 'every code-behind control whose handler needs a role is bound to that role' (@($audit823 | Where-Object { $_.Kind -eq 'Click' -and $_.Role }).Count -ge 10 -and $clickWrong823.Count -eq 0) -Severity Critical -Details ($clickWrong823 -join ' / ')
...
Add-MystTiqCheck $ctx 'v0.8.23.0 Contracts' 'the harnesses check every gated command and every page''s controls for each role (offline, and signed in to the VM)' `
...
Test-MystTiqFile $ctx 'docs\architecture\v0.8.23.0-every-button-follows-the-role.md' 'v0.8.23.0 Contracts' 'v0.8.23.0 architecture doc is present' -Severity Critical

# ---------------------------------------------------------------------------
# 4. Documentation
# ---------------------------------------------------------------------------
$docText = (Get-ChildItem (Join-Path $root 'docs') -Recurse -File -Include *.md | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n"
Add-MystTiqCheck $ctx 'Documentation' 'v0.8.23.0 is documented' ([regex]::IsMatch($docText, 'v0\.8\.23\.0')) -Severity High
Test-MystTiqFile $ctx 'release-notes\v0.8.23.0.md' 'Documentation' 'v0.8.23.0 release notes exist' -Severity High
$changelogText = Get-MystTiqText $ctx 'CHANGELOG.md'
Add-MystTiqCheck $ctx 'Documentation' 'CHANGELOG.md has a v0.8.23.0 entry' ($changelogText -match '## v0\.8\.23\.0') -Severity High
...
# ---------------------------------------------------------------------------
if ($RunBuild) {
    $frozenZip = if ($FrozenBaselineZip) { $FrozenBaselineZip } else { Join-Path $root '..\_Backups\MystTiqPalworldServer\v0.8.22.0\MystTiqPalworldServer_v0.8.22.0_FullSource.zip' }
    if (-not (Test-Path $frozenZip -PathType Leaf)) {
        # v0.8.1.0: reported, not fatal: the rest of the gate is still worth running on a machine without it.
        Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.22.0 checkpoint logic gate still passes' $false -Skipped -Severity Critical `
            -Details "Baseline archive not found at $frozenZip. Run on the release machine, or pass -FrozenBaselineZip <path>."
    }
    else {
    $frozenRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.22.0-frozen-baseline'
    ...
    $frozenGateLog = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.22.0-frozen-gate-output.txt'
    try {
        & (Join-Path $frozenProjectRoot 'scripts\Test-v0.8.22.0-Logic.ps1') -ProjectRoot $frozenProjectRoot *> $frozenGateLog
    } catch { }
    ...
    Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.22.0 checkpoint logic gate still passes' `
    ...
    Test-MystTiqCommand $ctx 'v0.8.23.0 Runtime' 'MystTiq.ArtworkHarness passes (incl. every gated command and page control for local use and each role)' {
