...
$ctx = New-MystTiqTestContext -ProjectRoot $root -Version '0.8.11.0' -Suite 'Logic'

# ---------------------------------------------------------------------------
# 1. Version / build identity
# ---------------------------------------------------------------------------
Test-MystTiqTextMatch $ctx 'Directory.Build.props' `
    '<VersionPrefix>0\.8\.11\.0</VersionPrefix>' `
    'Versioning' 'Directory.Build.props reports v0.8.11.0' -Severity Critical

Test-MystTiqTextMatch $ctx 'src\PalworldManager\app.manifest' `
    'assemblyIdentity version="0\.8\.11\.0"' `
    'Versioning' 'app.manifest reports v0.8.11.0' -Severity Critical
...
Test-MystTiqFile $ctx 'scripts\Test-v0.8.10.0-Logic.ps1' 'Regression' 'v0.8.10.0 logic gate remains available' -Severity High
...
# ---------------------------------------------------------------------------
# 3. v0.8.11.0 Contracts -- owned Pals' positions on the World Map
# ---------------------------------------------------------------------------
...
Add-MystTiqCheck $ctx 'v0.8.11.0 Contracts' 'Pals are placed by their container: a base worker container, a small (party) container, or the Palbox, which is only counted' `
...
Add-MystTiqCheck $ctx 'v0.8.11.0 Contracts' 'unset positions (0,0, placeholder altitude, within 20 m of the origin) are dropped, and property names match exactly (ID is not id)' `
...
Add-MystTiqCheck $ctx 'v0.8.11.0 Contracts' 'the explorer snapshot carries the Pals (named by owner and guild) and a summary, and a read failure degrades to none' `
...
Add-MystTiqCheck $ctx 'v0.8.11.0 Contracts' 'the map clusters Pals within 10 px, badges a cluster beside a base on its corner, and zooms a group all the way in' `
...
Add-MystTiqCheck $ctx 'v0.8.11.0 Contracts' 'Pals draw above bases and under players, in a fixed violet, and name labels step around them' `
...
Add-MystTiqCheck $ctx 'v0.8.11.0 Contracts' 'the logic harness covers the reader and the map layer (clusters, badge, label obstacles, status text)' `
...
Add-MystTiqCheck $ctx 'v0.8.11.0 Contracts' 'the Pal positions smoke uses its own FleetRoot and a synthetic save' `
    ($smoke -match '\$cfg\.FleetRoot = \$fleetRoot' -and $smoke -match 'PalPositionSmokeWorld') -Severity Critical
Test-MystTiqFile $ctx 'docs\architecture\v0.8.11.0-pal-positions.md' 'v0.8.11.0 Contracts' 'v0.8.11.0 architecture doc is present' -Severity Critical

# ---------------------------------------------------------------------------
# 4. Documentation
# ---------------------------------------------------------------------------
$docText = (Get-ChildItem (Join-Path $root 'docs') -Recurse -File -Include *.md | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n"
Add-MystTiqCheck $ctx 'Documentation' 'v0.8.11.0 is documented' ([regex]::IsMatch($docText, 'v0\.8\.11\.0')) -Severity High
Test-MystTiqFile $ctx 'release-notes\v0.8.11.0.md' 'Documentation' 'v0.8.11.0 release notes exist' -Severity High
$changelogText = Get-MystTiqText $ctx 'CHANGELOG.md'
Add-MystTiqCheck $ctx 'Documentation' 'CHANGELOG.md has a v0.8.11.0 entry' ($changelogText -match '## v0\.8\.11\.0') -Severity High
...
# ---------------------------------------------------------------------------
if ($RunBuild) {
    $frozenZip = if ($FrozenBaselineZip) { $FrozenBaselineZip } else { Join-Path $root '..\_Backups\MystTiqPalworldServer\v0.8.10.0\MystTiqPalworldServer_v0.8.10.0_FullSource.zip' }
    if (-not (Test-Path $frozenZip -PathType Leaf)) {
        # v0.8.1.0: reported, not fatal: the rest of the gate is still worth running on a machine without it.
        Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.10.0 checkpoint logic gate still passes' $false -Skipped -Severity Critical `
            -Details "Baseline archive not found at $frozenZip. Run on the release machine, or pass -FrozenBaselineZip <path>."
    }
    else {
    $frozenRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.10.0-frozen-baseline'
    ...
    $frozenGateLog = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.10.0-frozen-gate-output.txt'
    try {
        & (Join-Path $frozenProjectRoot 'scripts\Test-v0.8.10.0-Logic.ps1') -ProjectRoot $frozenProjectRoot *> $frozenGateLog
    } catch { }
    ...
    Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.10.0 checkpoint logic gate still passes' `
    ...
    Test-MystTiqCommand $ctx 'v0.8.11.0 Runtime' 'v0.8.11.0 Pal positions route smoke passes' {
