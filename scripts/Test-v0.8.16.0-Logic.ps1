...
$ctx = New-MystTiqTestContext -ProjectRoot $root -Version '0.8.16.0' -Suite 'Logic'

# ---------------------------------------------------------------------------
# 1. Version / build identity
# ---------------------------------------------------------------------------
Test-MystTiqTextMatch $ctx 'Directory.Build.props' `
    '<VersionPrefix>0\.8\.16\.0</VersionPrefix>' `
    'Versioning' 'Directory.Build.props reports v0.8.16.0' -Severity Critical

Test-MystTiqTextMatch $ctx 'src\PalworldManager\app.manifest' `
    'assemblyIdentity version="0\.8\.16\.0"' `
    'Versioning' 'app.manifest reports v0.8.16.0' -Severity Critical
...
Test-MystTiqFile $ctx 'scripts\Test-v0.8.15.0-Logic.ps1' 'Regression' 'v0.8.15.0 logic gate remains available' -Severity High
...
# ---------------------------------------------------------------------------
# 3. v0.8.16.0 Contracts -- theme modes (Midnight, High contrast, Follow the system) and density
# ---------------------------------------------------------------------------
...
Add-MystTiqCheck $ctx 'v0.8.16.0 Contracts' 'five modes on the two hand-tuned palettes: Dark and Light as before, Midnight and High contrast on Dark, System follows the OS; unknown is Dark' `
...
Add-MystTiqCheck $ctx 'v0.8.16.0 Contracts' 'card borders and the base control, card, box and list-row sizes are resources, so modes and density reach them; class-specific styles keep their own' `
...
Add-MystTiqCheck $ctx 'v0.8.16.0 Contracts' 'the mode picker replaces the Light mode checkbox (per tab); density applies to every tab and is remembered; a tab following the system changes with it' `
...
Add-MystTiqCheck $ctx 'v0.8.16.0 Contracts' 'the ArtworkHarness checks every mode (stored per tab, contrast ratios, true black, white borders), following the system both ways, and density (isolated store)' `
...
Test-MystTiqFile $ctx 'docs\architecture\v0.8.16.0-theme-modes.md' 'v0.8.16.0 Contracts' 'v0.8.16.0 architecture doc is present' -Severity Critical

# ---------------------------------------------------------------------------
# 4. Documentation
# ---------------------------------------------------------------------------
$docText = (Get-ChildItem (Join-Path $root 'docs') -Recurse -File -Include *.md | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n"
Add-MystTiqCheck $ctx 'Documentation' 'v0.8.16.0 is documented' ([regex]::IsMatch($docText, 'v0\.8\.16\.0')) -Severity High
Test-MystTiqFile $ctx 'release-notes\v0.8.16.0.md' 'Documentation' 'v0.8.16.0 release notes exist' -Severity High
$changelogText = Get-MystTiqText $ctx 'CHANGELOG.md'
Add-MystTiqCheck $ctx 'Documentation' 'CHANGELOG.md has a v0.8.16.0 entry' ($changelogText -match '## v0\.8\.16\.0') -Severity High
...
# ---------------------------------------------------------------------------
if ($RunBuild) {
    $frozenZip = if ($FrozenBaselineZip) { $FrozenBaselineZip } else { Join-Path $root '..\_Backups\MystTiqPalworldServer\v0.8.15.0\MystTiqPalworldServer_v0.8.15.0_FullSource.zip' }
    if (-not (Test-Path $frozenZip -PathType Leaf)) {
        # v0.8.1.0: reported, not fatal: the rest of the gate is still worth running on a machine without it.
        Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.15.0 checkpoint logic gate still passes' $false -Skipped -Severity Critical `
            -Details "Baseline archive not found at $frozenZip. Run on the release machine, or pass -FrozenBaselineZip <path>."
    }
    else {
    $frozenRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.15.0-frozen-baseline'
    ...
    $frozenGateLog = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.15.0-frozen-gate-output.txt'
    try {
        & (Join-Path $frozenProjectRoot 'scripts\Test-v0.8.15.0-Logic.ps1') -ProjectRoot $frozenProjectRoot *> $frozenGateLog
    } catch { }
    ...
    Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.15.0 checkpoint logic gate still passes' `
    ...
    Test-MystTiqCommand $ctx 'v0.8.16.0 Runtime' 'MystTiq.ArtworkHarness passes (offline headless rendering, incl. every theme mode and density)' {
