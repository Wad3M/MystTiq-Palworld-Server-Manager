...
$ctx = New-MystTiqTestContext -ProjectRoot $root -Version '0.8.25.0' -Suite 'Logic'

# ---------------------------------------------------------------------------
# 1. Version / build identity
# ---------------------------------------------------------------------------
Test-MystTiqTextMatch $ctx 'Directory.Build.props' `
    '<VersionPrefix>0\.8\.25\.0</VersionPrefix>' `
    'Versioning' 'Directory.Build.props reports v0.8.25.0' -Severity Critical

Test-MystTiqTextMatch $ctx 'src\PalworldManager\app.manifest' `
    'assemblyIdentity version="0\.8\.25\.0"' `
    'Versioning' 'app.manifest reports v0.8.25.0' -Severity Critical
...
Test-MystTiqFile $ctx 'scripts\Test-v0.8.24.0-Logic.ps1' 'Regression' 'v0.8.24.0 logic gate remains available' -Severity High
...
$gateSelf821 = Get-MystTiqText $ctx 'scripts\Test-v0.8.25.0-Logic.ps1'
...
# ---------------------------------------------------------------------------
# 3. v0.8.25.0 Contracts -- every colour follows the mode, and Windows contrast themes
# ---------------------------------------------------------------------------
...
Add-MystTiqCheck $ctx 'v0.8.25.0 Contracts' 'no literal colour is left in the styles or the window (outside comments and keyed resources): each is a decorative resource the mode derives, shadows included' `
...
Add-MystTiqCheck $ctx 'v0.8.25.0 Contracts' 'Windows contrast themes are read from Windows (SPI_GETHIGHCONTRAST, GetSysColor) and used by High contrast and Follow the system, live; buttons use the theme''s button text' `
...
Add-MystTiqCheck $ctx 'v0.8.25.0 Contracts' 'the ArtworkHarness checks the decorative colours in every mode and a dark and a light Windows contrast theme (this machine''s own setting pinned off)' `
...
Add-MystTiqCheck $ctx 'v0.8.25.0 Contracts' 'the HOST tab has its own page art (day and night files) and navigation icon' `
...
Test-MystTiqFile $ctx 'docs\architecture\v0.8.25.0-theme-leftovers.md' 'v0.8.25.0 Contracts' 'v0.8.25.0 architecture doc is present' -Severity Critical

# ---------------------------------------------------------------------------
# 4. Documentation
# ---------------------------------------------------------------------------
$docText = (Get-ChildItem (Join-Path $root 'docs') -Recurse -File -Include *.md | ForEach-Object { Get-Content $_.FullName -Raw }) -join "`n"
Add-MystTiqCheck $ctx 'Documentation' 'v0.8.25.0 is documented' ([regex]::IsMatch($docText, 'v0\.8\.25\.0')) -Severity High
Test-MystTiqFile $ctx 'release-notes\v0.8.25.0.md' 'Documentation' 'v0.8.25.0 release notes exist' -Severity High
$changelogText = Get-MystTiqText $ctx 'CHANGELOG.md'
Add-MystTiqCheck $ctx 'Documentation' 'CHANGELOG.md has a v0.8.25.0 entry' ($changelogText -match '## v0\.8\.25\.0') -Severity High
...
# ---------------------------------------------------------------------------
if ($RunBuild) {
    $frozenZip = if ($FrozenBaselineZip) { $FrozenBaselineZip } else { Join-Path $root '..\_Backups\MystTiqPalworldServer\v0.8.24.0\MystTiqPalworldServer_v0.8.24.0_FullSource.zip' }
    if (-not (Test-Path $frozenZip -PathType Leaf)) {
        # v0.8.1.0: reported, not fatal: the rest of the gate is still worth running on a machine without it.
        Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.24.0 checkpoint logic gate still passes' $false -Skipped -Severity Critical `
            -Details "Baseline archive not found at $frozenZip. Run on the release machine, or pass -FrozenBaselineZip <path>."
    }
    else {
    $frozenRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.24.0-frozen-baseline'
    ...
    $frozenGateLog = Join-Path ([System.IO.Path]::GetTempPath()) 'MystTiq-v0.8.24.0-frozen-gate-output.txt'
    try {
        & (Join-Path $frozenProjectRoot 'scripts\Test-v0.8.24.0-Logic.ps1') -ProjectRoot $frozenProjectRoot *> $frozenGateLog
    } catch { }
    ...
    Add-MystTiqCheck $ctx 'Regression Logic' 'Frozen v0.8.24.0 checkpoint logic gate still passes' `
    ...
    Test-MystTiqCommand $ctx 'v0.8.25.0 Runtime' 'MystTiq.ArtworkHarness passes (incl. the decorative colours in every mode and Windows contrast themes)' {
