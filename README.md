...
> **Current release candidate:** v0.7.90.0 — Install With Extras (UE4SS)
...
| **v0.7.90.0** | **Current RC** | Install With Extras (UE4SS) — direct live feedback: "we should add that as part of the install, option to install just the server without mods and then the option of installing it with the extras." The wizard's Install step now offers "Install Palworld Dedicated Server" (server only, unchanged) alongside a new "Install with Extras (+ UE4SS)", which also auto-installs the newest stable UE4SS release via the same Preview/Apply flow the UE4SS page already uses. Palworld Save Tools and PIM/Oodle Decoder remain out of scope, confirmed neither has any install capability anywhere in the app yet. Touches `MystTiq.Desktop` only |
