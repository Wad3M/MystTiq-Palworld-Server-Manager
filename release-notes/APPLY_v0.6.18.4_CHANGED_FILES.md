# Apply v0.6.18.4 Changed Files

The Changed Files ZIP is an audit/convenience package for the promoted v0.6.18.4 source baseline. Normal installation should use `MystTiqPalworldServer_v0.6.18.4_FullSource.zip` with `Update-FromDownloads.ps1` so the running artifact app closes, the staged source replaces cleanly, and the complete current-version gate runs before relaunch.
