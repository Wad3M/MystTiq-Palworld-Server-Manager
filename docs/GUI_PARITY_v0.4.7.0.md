# v0.4.7.0 GUI parity audit — legacy v0.2.16.4 vs Avalonia
...
| Legacy area | v0.4.7.0 status | Avalonia/headless state |
|---|---|---|
...
v0.4.7.0 closes the first reconstruction phase for **Dashboard + Server Setup**. Dashboard cards restore direct OPEN/BACKUP/DOCTOR workflows and the legacy health strip while keeping RCON/Cleanup visibly BACKEND REQUIRED where no safe current service exists. Server Setup now mirrors the historical banner/checklist/operation-monitor hierarchy and every environment action carries explicit backend capability metadata so unsupported installs cannot become decorative no-op buttons.
