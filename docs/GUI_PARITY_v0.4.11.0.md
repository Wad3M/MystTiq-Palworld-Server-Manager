# v0.4.11.0 GUI parity audit — legacy v0.2.16.4 vs Avalonia
...
| Legacy area | v0.4.11.0 status | Avalonia/headless state |
|---|---|---|
...
v0.4.11.0 restores **Guilds + Bases parity** for safe read-only operations. Authoritative decoded guild IDs, leadership, member IDs, health, and `base_ids` flow through the existing headless explorer endpoint. The Avalonia client supplies search/status filters, CSV export, detail panes, ID copy, and guild-leader player navigation without opening server-local files.

State-changing leader/member/orphan/base repair and recovery controls remain visibly BACKEND REQUIRED. Their documented enablement condition is the full Preview → Safety Backup → Server-side Transaction → Validate → Journal/Audit → Refresh GUI chain.

## v0.4.11.0 Configuration parity status
