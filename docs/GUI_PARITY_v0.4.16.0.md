# v0.4.16.0 GUI parity audit — legacy v0.2.16.4 vs Avalonia
...
| Legacy area | v0.4.16.0 status | Avalonia/headless state |
|---|---|---|
...
v0.4.16.0 restores Activity/Audit filtering/export and persistent Notifications parity. Filtering and export remain local presentation operations; the headless service owns append-only evidence and bounded atomic notification state. Read, pin, dismiss and self-test operations flow through the authenticated API and are audited without persisting private message bodies in audit details.
