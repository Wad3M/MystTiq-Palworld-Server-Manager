# v0.5.1.1 final legacy GUI event coverage

The authoritative row-level report is `docs/reconstruction/MystTiq_v0.2.16.4_GUI_Event_Coverage_v0.5.1.1.csv`. It contains all 287 rows from the final v0.2.16.4 WPF event inventory, in original order, with a classification, current mapping and evidence for every row.
