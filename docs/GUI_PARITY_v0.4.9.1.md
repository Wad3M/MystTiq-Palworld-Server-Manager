# v0.4.9.1 GUI parity audit — legacy v0.2.16.4 vs Avalonia
...
| Legacy area | v0.4.9.1 status | Avalonia/headless state |
|---|---|---|
...
v0.4.9.1 restores **Console + RCON parity**. Console presentation gains the historical filters/search/pause/clear/export workflow without altering persistent server logs. RCON is implemented server-side through MystTiq's management API and Core Source RCON service; the desktop never receives AdminPassword or opens an RCON socket. Global card containment also becomes a shared Avalonia layout rule so dynamic controls cannot render outside card boundaries.

## v0.4.9.1 Configuration parity status
